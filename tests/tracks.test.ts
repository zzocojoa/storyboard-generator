...
describe('독립 오디오·글자 트랙 편집', (): void => {
  ...
  it('제안 글자 큐는 옮기되 원본에서 확정된 자막 시작점은 거부한다', async (): Promise<void> => {
    ...
    const movable: TextCue = { ...fixed, id: 'movable-text', placementId: null, timingStatus: 'proposed' };
    ...
    expect(() => updateTextCueTiming(project, fixed.id, { startMs: fixed.startMs + 1, endMs: fixed.endMs, kind: fixed.kind })).toThrowError(expect.objectContaining({ code: 'INVALID_TRACK_EDIT' }));
  });
});
