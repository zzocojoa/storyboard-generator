...
describe('Canonical Generation Audit', (): void => {
  ...
  it('legacy_generation_record_migrates_build_to_null', async (): Promise<void> => {
    ...
    expect(migrated.generationRecords).toEqual([{ ...legacy, generatorBuild: null }]); expect(migrated.schemaVersion).toBe('1.23.0'); expect(parseProject(migrated)).toEqual(migrated);
  });
});
