...
export async function referenceRetakeCandidate(source: Project): Promise<AutomaticReferenceCandidate> {
  const project = compileAutomaticProductionPlan(source, createProductionPlanBasis(source, ['demonstration']), {
    schemaVersion: '1.0.0', profile: { ...source.profile, medium: 'ai', visualStyle: '밝은 연필 콘티' }, profileReason: '화분 시연용 합성 제작 기준',
    resources: [{ key: 'bench', kind: 'location', subjectId: 'workbench', name: '밝은 작업대', description: '왼쪽 창문 앞의 나무 작업대', reason: '화분 시연 장소', sourceRefs: source.dataset.locations[0]!.sourceRefs, sourceUnitIds: [], referenceAssetId: null }],
