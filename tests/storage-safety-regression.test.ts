...
async function crashedUpdate(point: StorageFaultPoint, withAsset: boolean): Promise<CrashFixture> {
  ...
  const project: Project = await trackedStore(dataRoot).create(await outline());
  ...
  const store = trackedStore(dataRoot, injector(point));
...
async function versionCollision(sameHash: boolean, withAsset: boolean): Promise<{ dataRoot: string; project: Project; versionPath: string; assetPath: string | null; versionIdentity: string }> {
  ...
  const store = trackedStore(dataRoot); const project: Project = await store.create(await outline());
...
async function crashedCreate(point: StorageFaultPoint): Promise<CrashFixture> {
  ...
  const project: Project = await outline(`create-${point}`); const store = trackedStore(dataRoot, injector(point));
...
async function blockFixture(): Promise<CrashFixture> {
  ...
  await trackedStore(fixture.dataRoot).initialize();
  return fixture;
}
...
describe('A. Update preflight collisions', (): void => {
  ...
  it('preflight_failure_keeps_current_revision', async (): Promise<void> => { const fixture = await assetCollision(); expect((await trackedStore(fixture.dataRoot).read(fixture.project.projectId)).revision).toBe(0); });
...
describe('B. Transaction file ownership', (): void => {
  ...
  it('same_hash_different_inode_is_not_transaction_owned', async (): Promise<void> => {
    ...
    await unlink(finalPath); await writeFile(finalPath, fixture.bytes as Buffer); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()).toHaveLength(1);
  });
  ...
  it('rollback_deletes_owned_asset', async (): Promise<void> => {
    ...
    await trackedStore(fixture.dataRoot).initialize(); expect(await exists(path)).toBe(false);
  });
  it('rollback_does_not_delete_unowned_same_hash_asset', async (): Promise<void> => {
    const fixture = await crashedUpdate('after-update-asset-linked', true); const path = join(projectDirectory(fixture.dataRoot, fixture.project.projectId), (fixture.asset as Asset).path);
    await unlink(path); await writeFile(path, fixture.bytes as Buffer); await trackedStore(fixture.dataRoot).initialize(); expect(await exists(path)).toBe(true);
  });
  ...
  it('rollback_does_not_delete_unowned_same_hash_version', async (): Promise<void> => {
    const fixture = await crashedUpdate('after-update-version-linked', false); const path = join(projectDirectory(fixture.dataRoot, fixture.project.projectId), 'versions', '000001.json');
    const bytes: Buffer = await readFile(path); await unlink(path); await writeFile(path, bytes); await trackedStore(fixture.dataRoot).initialize(); expect(await exists(path)).toBe(true);
  });
  ...
  it('committed_transaction_removes_staged_hardlinks', async (): Promise<void> => { const fixture = await crashedUpdate('before-update-cleanup', true); await trackedStore(fixture.dataRoot).initialize(); expect(await exists(fixture.transactionPath as string)).toBe(false); });
  it('ambiguous_file_identity_returns_recovery_required', async (): Promise<void> => { const fixture = await crashedUpdate('after-update-version-linked', false); const path = join(projectDirectory(fixture.dataRoot, fixture.project.projectId), 'versions', '000001.json'); const bytes = await readFile(path); await unlink(path); await writeFile(path, bytes); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()[0]?.code).toBe('STORE_RECOVERY_REQUIRED'); });
});

describe('C. All-version asset references', (): void => {
  it('current_project_reference_blocks_asset_delete', async (): Promise<void> => {
    const fixture = await referenceCrash(); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize();
  ...
  it('version_zero_reference_blocks_asset_delete', async (): Promise<void> => {
    const fixture = await referenceCrash(); await trackedStore(fixture.dataRoot).initialize();
  ...
  it('historical_version_reference_blocks_asset_delete', async (): Promise<void> => {
    const fixture = await historicalReferenceCrash(); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize();
    expect(recovered.recoveryBlocks()).toHaveLength(1); expect(await exists(join(projectDirectory(fixture.dataRoot, fixture.project.projectId), (fixture.asset as Asset).path))).toBe(true);
  });
  ...
  it('other_transaction_previous_reference_blocks_delete', async (): Promise<void> => {
    ...
    const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()).toHaveLength(1);
  ...
  it('excluded_owned_next_version_does_not_false_block_rollback', async (): Promise<void> => {
    const fixture = await crashedUpdate('after-update-version-linked', true); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize();
  ...
  it('unreferenced_owned_asset_can_be_removed', async (): Promise<void> => {
    const fixture = await crashedUpdate('after-update-asset-linked', true); await trackedStore(fixture.dataRoot).initialize();
...
describe('D. Initial create consistency', (): void => {
  it('committed_create_requires_current_and_version_zero', async (): Promise<void> => {
    ...
    const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()[0]?.code).toBe('STORE_CREATE_RECOVERY_REQUIRED');
  });
  ...
  it('final_create_hash_mismatch_is_not_silently_cleaned', async (): Promise<void> => {
    ...
    await writeCompleteProject(fixture.dataRoot, concurrent); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize();
  ...
  it('incomplete_final_directory_preserves_create_journal', async (): Promise<void> => {
    ...
    const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()).toHaveLength(1); expect(await exists(fixture.transactionPath as string)).toBe(true);
  });
  ...
  it('concurrent_complete_create_is_proven_and_preserved', async (): Promise<void> => {
    ...
    await writeCompleteProject(fixture.dataRoot, concurrent); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect((await recovered.read(concurrent.projectId)).title).toBe('proven concurrent');
  });
  ...
  it('concurrent_complete_create_cleans_only_own_staging', async (): Promise<void> => {
    ...
    await trackedStore(fixture.dataRoot).initialize(); expect(await exists(fixture.transactionPath as string)).toBe(false); expect(await exists(join(directory, 'project.json'))).toBe(true);
  });
  ...
  it('complete_create_asset_references_are_verified', async (): Promise<void> => {
    ...
    await writeCompleteProject(fixture.dataRoot, parseProject({ ...fixture.project, assets: [asset] })); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize();
  ...
  it('create_recovery_is_idempotent', async (): Promise<void> => {
    const fixture = await crashedCreate('after-create-directory-publish'); const first = trackedStore(fixture.dataRoot); await first.initialize(); const second = trackedStore(fixture.dataRoot); await second.initialize();
  ...
  it('failed_create_can_be_retried_after_safe_rollback', async (): Promise<void> => {
    const fixture = await crashedCreate('before-create-directory-publish'); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize();
  ...
  it('create_internal_directory_is_not_listed_as_project', async (): Promise<void> => {
    const fixture = await crashedCreate('after-create-journal-prepared'); const store = trackedStore(fixture.dataRoot); await store.initialize(); expect(await store.list()).toEqual([]);
  });
});

describe('E. Symlink safety', (): void => {
  it('asset_directory_symlink_is_rejected', async (): Promise<void> => {
    const fixtureRoot: string = await root('storyboard-symlink-assets-'); const dataRoot: string = join(fixtureRoot, 'data'); const project = await trackedStore(dataRoot).create(await outline());
    ...
    const recovered = trackedStore(dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()[0]?.code).toBe('STORE_PATH_UNSAFE');
  });
  ...
  it('asset_file_symlink_is_rejected', async (): Promise<void> => {
    ...
    const store: ProjectStore = trackedStore(dataRoot); const base: Project = await store.create(await outline());
    ...
    await expect(trackedStore(dataRoot).asset(project.projectId, asset.id)).rejects.toSatisfy((error: unknown): boolean => codeOf(error) === 'STORED_ASSET_PATH_UNSAFE');
  });

  it('versions_directory_symlink_is_rejected', async (): Promise<void> => {
    const fixtureRoot: string = await root('storyboard-symlink-versions-'); const dataRoot: string = join(fixtureRoot, 'data'); const project = await trackedStore(dataRoot).create(await outline());
  ...
  it('transaction_directory_symlink_is_rejected', async (): Promise<void> => {
    const fixtureRoot: string = await root('storyboard-symlink-transactions-'); const dataRoot: string = join(fixtureRoot, 'data'); const project = await trackedStore(dataRoot).create(await outline());
  ...
  it('create_staging_symlink_is_rejected', async (): Promise<void> => {
    ...
    const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()[0]?.code).toBe('STORE_PATH_UNSAFE');
  });
  ...
  it('recovery_does_not_unlink_through_symlink', async (): Promise<void> => {
    ...
    await trackedStore(fixture.dataRoot).initialize(); expect((await readFile(outside)).equals(fixture.bytes as Buffer)).toBe(true);
  });
  ...
  it('safe_write_rejects_symlinked_parent', async (): Promise<void> => {
    const fixtureRoot: string = await root('storyboard-safe-write-'); const dataRoot: string = join(fixtureRoot, 'data'); const store = trackedStore(dataRoot); const project = await store.create(await outline());
  ...
  it('project_root_escape_is_rejected', async (): Promise<void> => {
    const fixtureRoot: string = await root('storyboard-root-escape-'); const dataRoot: string = join(fixtureRoot, 'data'); const store = trackedStore(dataRoot); const project = await store.create(await outline()); const bytes = await png(2, 2);
...
describe('F. Recovery-blocked mutation', (): void => {
  it('rollback_recovery_failure_blocks_next_update', async (): Promise<void> => {
    const fixture = await blockFixture(); const store = trackedStore(fixture.dataRoot); await store.initialize();
  ...
  it('blocked_project_rejects_asset_upload', async (): Promise<void> => {
    const fixture = await blockFixture(); const app = await appForStore(fixture.root, fixture.dataRoot, trackedStore(fixture.dataRoot));
  ...
  it('blocked_project_is_reported_in_status', async (): Promise<void> => {
    const fixture = await blockFixture(); const app = await appForStore(fixture.root, fixture.dataRoot, trackedStore(fixture.dataRoot)); const response = await app.inject({ method: 'GET', url: '/api/status' }); await app.close();
  ...
  it('successful_restart_recovery_clears_block', async (): Promise<void> => {
    ...
    await unlink(finalPath); await link(stagedPath, finalPath); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(recovered.recoveryBlocks()).toEqual([]); expect(await exists(fixture.transactionPath as string)).toBe(false);
  });

  it('unrelated_project_can_continue_when_project_scoped_block_is_used', async (): Promise<void> => {
    const fixtureRoot: string = await root('storyboard-project-scoped-'); const dataRoot: string = join(fixtureRoot, 'data'); const first: Project = await trackedStore(dataRoot).create(await outline('blocked-project')); const second: Project = await trackedStore(dataRoot).create(await outline('healthy-project'));
    const bytes: Buffer = await png(2, 2); const asset: Asset = assetMetadata('scoped-owned', bytes); const crashing = trackedStore(dataRoot, injector('after-update-asset-linked'));
    ...
    const finalPath: string = join(projectDirectory(dataRoot, first.projectId), asset.path); await unlink(finalPath); await writeFile(finalPath, bytes); const recovered = trackedStore(dataRoot); await recovered.initialize();
  ...
  it('recovery_block_survives_second_store_instance_or_persistent_lock', async (): Promise<void> => {
    const fixture = await blockFixture(); const second = trackedStore(fixture.dataRoot); await second.initialize(); expect(second.recoveryBlocks()).toHaveLength(1);
...
describe('H. Actual ProjectStore fault injection', (): void => {
  it('actual_update_crash_after_preflight_is_recovered', async (): Promise<void> => { const fixture = await crashedUpdate('after-update-preflight', false); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(0); expect(recovered.recoveryBlocks()).toEqual([]); });
  it('actual_update_crash_after_journal_is_recovered', async (): Promise<void> => { const fixture = await crashedUpdate('after-update-journal-prepared', false); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(0); expect(recovered.recoveryBlocks()).toEqual([]); });
  it('actual_update_crash_after_asset_link_is_recovered', async (): Promise<void> => { const fixture = await crashedUpdate('after-update-asset-linked', true); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(0); });
  it('actual_update_crash_after_version_link_is_recovered', async (): Promise<void> => { const fixture = await crashedUpdate('after-update-version-linked', true); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(0); });
  it('actual_update_crash_after_current_publish_is_committed', async (): Promise<void> => { const fixture = await crashedUpdate('after-update-current-published', true); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(1); });
  it('actual_update_crash_before_cleanup_is_committed', async (): Promise<void> => { const fixture = await crashedUpdate('before-update-cleanup', true); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(1); });
  it('actual_create_crash_after_journal_is_recovered', async (): Promise<void> => { const fixture = await crashedCreate('after-create-journal-prepared'); const recovered = trackedStore(fixture.dataRoot); await recovered.initialize(); expect(await recovered.list()).toEqual([]); });
  ...
  it('actual_create_crash_after_directory_publish_preserves_project', async (): Promise<void> => { const fixture = await crashedCreate('after-create-directory-publish'); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(0); });
  it('actual_create_crash_before_cleanup_preserves_project', async (): Promise<void> => { const fixture = await crashedCreate('before-create-cleanup'); const recovered = trackedStore(fixture.dataRoot); expect((await recovered.read(fixture.project.projectId)).revision).toBe(0); expect(await exists(fixture.transactionPath as string)).toBe(false); });
