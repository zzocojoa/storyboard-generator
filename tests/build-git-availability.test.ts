...
describe('Git 확인 불가 Provenance와 1.9 이관', (): void => {
  ...
  it('current_build_manifest_uses_provenance_version_3', (): void => {
    expect(readBuildManifest()).toMatchObject({ provenanceVersion: 3, gitStateAvailable: true, projectSchemaVersion: '1.21.0' });
  ...
  it('project_18_to_19_migration_is_idempotent', async (): Promise<void> => {
    ...
    expect(migrated.schemaVersion).toBe('1.21.0'); expect(parseProject(migrated)).toEqual(migrated);
  });
