...
test('e2e_producer_review_plays_pending_frames_in_order_and_preserves_final_and_project', async ({ page }): Promise<void> => {
  ...
  try {
    ...
    const keyEditor = page.locator('.frame-editor').filter({ has: page.getByRole('img', { name: 'key 프레임 생성 결과 검토', exact: true }) });
    await expect(keyEditor).toBeFocused();
    await expect(keyEditor.getByRole('img')).toBeInViewport();
    await expect(keyEditor.getByRole('textbox', { name: '프레임 설명', exact: true })).toHaveValue('중간 동작 검토');
    await page.screenshot({ path: '.local/validation/automation-runtime/frame-target-desktop.png' });
    await page.setViewportSize({ width: 390, height: 844 });
    await page.getByRole('button', { name: '초안 미리보기', exact: true }).click();
    await seek(13499);
    await monitor.getByRole('button', { name: '이 그림 편집', exact: true }).click();
    const endEditor = page.locator('.frame-editor').filter({ has: page.getByRole('img', { name: 'end 프레임 생성 결과 검토', exact: true }) });
    await expect(endEditor).toBeFocused();
    await expect(endEditor.getByRole('img')).toBeInViewport();
    await expect(endEditor.getByRole('textbox', { name: '프레임 설명', exact: true })).toHaveValue('마지막 동작 검토');
    await page.screenshot({ path: '.local/validation/automation-runtime/frame-target-mobile.png' });
