...
describe('PDF 콘티', (): void => {
  it('그림이 없는 초안도 모든 프레임을 포함한 PDF로 만든다', async (): Promise<void> => {
    ...
    const pdf: Buffer = await exportProjectPdf(project, '/System/Library/Fonts/Supplemental/AppleGothic.ttf', async (): Promise<Buffer> => { throw new Error('이미지 로더가 호출되면 안 됩니다.'); });
