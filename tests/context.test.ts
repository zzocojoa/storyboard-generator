...
import { buildFrameImageContext, buildImageContext, buildSegmentContext } from '../src/proposal/context.js';
...
describe('생성 요청의 정보 경계', (): void => {
  it('후반 원문·전체 파일·인물 비밀을 현재 구간의 생성 입력에 전달하지 않는다', async (): Promise<void> => {
    ...
    const shot = project.shots.find((candidate) => candidate.id === 'shot-6');
    if (shot === undefined) throw new Error('이미지 문맥 검증용 컷이 없습니다.');
    const screenLink = shot.sourceLinks.find((link): boolean => link.unitId === 'UNIT-017');
    if (screenLink === undefined) throw new Error('이미지 문맥 검증용 화면 문구 연결이 없습니다.');
    const frameId: string = 'context-late-frame';
    const image = buildFrameImageContext({ ...project,
      shots: project.shots.map((candidate) => candidate.id === shot.id ? { ...candidate, sourceLinks: [screenLink] } : candidate),
      frames: [...project.frames, { id: frameId, shotId: shot.id, offsetMs: 18000, role: 'key', description: '정보 공개 뒤 프레임', imageAssetId: null, visualReview: 'pending' }],
    }, frameId);
  ...
  it('후반 원문을 앞선 컷에 연결하거나 정보 ID 목록을 비워도 공개 검사를 우회할 수 없다', async (): Promise<void> => {
    ...
    const foreign = { ...project, shots: project.shots.map((shot) => shot.id === 'shot-1' ? { ...shot, sourceLinks: [{ unitId: 'UNIT-067', usage: 'primary-visual' as const, status: 'confirmed' as const, temporalAnchor: { kind: 'shot-offset' as const, startOffsetMs: 0, endOffsetMs: 1, basis: 'manual' as const, status: 'confirmed' as const } }], informationIds: [] } : shot) };
    expect(() => buildImageContext(foreign, 'shot-1')).toThrowError(expect.objectContaining({ code: 'FRAME_GENERATION_BLOCKED' }));
    ...
    expect(() => buildImageContext(forged, 'shot-1')).toThrowError(expect.objectContaining({ code: 'FRAME_GENERATION_BLOCKED' }));
  });
});
