...
describe('생성 요청의 정보 경계', (): void => {
  it('후반 원문·전체 파일·인물 비밀을 현재 구간의 생성 입력에 전달하지 않는다', async (): Promise<void> => {
    ...
    const image = buildFrameImageContext({ ...project, frames: [...project.frames, { id: frameId, shotId: shot.id, offsetMs: 18000, role: 'key', description: '정보 공개 뒤 프레임', imageAssetId: null, visualReview: 'pending' }] }, frameId);
  ...
  it('후반 원문을 앞선 컷에 연결하거나 정보 ID 목록을 비워도 공개 검사를 우회할 수 없다', async (): Promise<void> => {
    ...
    const foreign = { ...project, shots: project.shots.map((shot) => shot.id === 'shot-1' ? { ...shot, sourceLinks: [{ unitId: 'UNIT-067', usage: 'primary-visual' as const, status: 'confirmed' as const }], informationIds: [] } : shot) };
