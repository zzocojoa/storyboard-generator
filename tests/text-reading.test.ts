...
it('text_reading_migrates_old_snapshot_without_rewriting_original_and_rejects_undeclared_settings', async (): Promise<void> => {
  ...
  expect(evidence.project).toEqual({ ...project, textLayoutControl: { version: '1.0.0', mode: 'manual', plannedInputHash: null } }); expect(evidence.projectionHashes).toHaveLength(12); expect(JSON.stringify(legacy)).toBe(bytes);
