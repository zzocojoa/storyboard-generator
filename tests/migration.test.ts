...
describe('프로젝트 형식 마이그레이션', (): void => {
  it('1.0 저장본의 컷을 명시적 CUT 전환이 있는 1.1 형식으로 읽는다', async (): Promise<void> => {
    ...
    const legacy = JSON.parse(JSON.stringify(project)) as { schemaVersion: string; shots: { [key: string]: unknown }[] };
    legacy.schemaVersion = '1.0.0';
    for (const shot of legacy.shots) delete shot.transitionOut;
