...
it('automatic_text_preset_legacy_migration_keeps_file_hashes_and_treats_unknown_ownership_as_manual', async (): Promise<void> => {
  ...
  try {
    ...
    expect(evidence.project.schemaVersion).toBe('1.21.0');
    ...
    expect(JSON.parse(await readFile(path, 'utf8'))).toMatchObject({ schemaVersion: '1.21.0', revision: 1, textLayoutControl: { mode: 'manual' } });
