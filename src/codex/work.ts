...
function proposalPrompt(context: SegmentContext): string {
  return `현재 구간의 원문과 제작 지시만 사용해 촬영 가능한 컷 제안을 작성하세요. sourceLinks로 모든 원문 단위를 빠짐없이 연결하고 primary-visual, continued-visual, audio-only, context-only 중 용도를 지정하세요. 원문 order를 컷 순서와 anchor 시작 시각에서 역전하거나 원문을 바꾸지 마세요. 같은 원문을 반복할 때는 두 번째부터 continued-visual 또는 context-only를 사용하세요. 각 sourceLink의 선택적 anchor는 컷 내부 상대 범위이며 0 ≤ startPermille < endPermille ≤ 1000이어야 합니다. anchor가 없으면 컷 전체입니다. 긴 컷 후반의 공개는 예를 들어 { "startPermille": 700, "endPermille": 1000 }처럼 작성하고, 실제 anchor 시작 시각이 Information Gate보다 이르지 않게 하세요. durationWeight는 컷 예상 길이의 상대 비율입니다. 각 컷의 transitionOut을 지정하세요. CUT은 durationMs 0, 그 밖의 전환은 컷 길이 안의 양수 밀리초로 작성하세요. 결과는 SegmentProposal 스키마에 맞는 JSON이어야 합니다.\n\n${JSON.stringify(context, null, 2)}`;
}
