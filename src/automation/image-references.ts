...
export async function verifiedImageReferences(project: Project, ids: readonly string[], loaded: readonly LoadedReference[], label: (asset: Asset) => string): Promise<ImageGenerationReference[]> {
  const assets: Asset[] = ids.map((id): Asset => {
    const asset = project.assets.find((candidate): boolean => candidate.id === id && ['character', 'location', 'prop'].includes(candidate.kind));
    if (asset === undefined) throw contractError('ASSET_REFERENCE_NOT_FOUND', `시각 기준 자산이 없습니다: ${id}`, []);
    return asset;
  });
  return verifiedAssetImageReferences(assets, loaded, label);
}

/** 호출자가 현재 생성 근거로 선택한 자산의 순서와 실제 이미지 바이트를 검증한다. */
export async function verifiedAssetImageReferences(assets: readonly Asset[], loaded: readonly LoadedReference[], label: (asset: Asset) => string): Promise<ImageGenerationReference[]> {
  assertReferenceBudget(loaded);
  const ids: string[] = assets.map((asset): string => asset.id);
  if (new Set(ids).size !== ids.length) throw contractError('AUTOMATION_REFERENCE_INPUT', '선택한 이미지 자산 ID가 중복됩니다.', []);
  ...
  for (const asset of assets) {
    ...
    if (!['character', 'location', 'prop', 'image'].includes(asset.kind)) throw contractError('ASSET_REFERENCE_NOT_FOUND', `이미지 참조 종류가 아닙니다: ${id}`, []);
