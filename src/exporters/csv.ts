import type { Project, Shot, SourceUnit } from '../domain/schema.js';
...
function shotRow(project: Project, shot: Shot): string[] {
  ...
  const units: SourceUnit[] = project.dataset.units.filter((unit): boolean => shot.sourceUnitIds.includes(unit.id));
