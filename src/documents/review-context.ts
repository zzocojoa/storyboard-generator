...
export function documentReviewPrompt(sources: DocumentSources, preview: DocumentPreview, input: StoredReviewInput): string {
  return [
    ...
    '아래 JSON의 sources는 선택한 8개 파일 전체입니다. 이 데이터만 근거로 사용하세요.',
    JSON.stringify({ preview, current: { bindings: input.input.bindings, production: input.input.production }, preset: input.preset,
