...
공통 음향 수정 커밋 `f26fb8a5cf652c963f728a100a9be8576d284a9c`의 [CI 34685873801](https://github.com/zzocojoa/storyboard-generator/actions/runs/34685873801)는 `check`를 통과했고 E2E는 72개 중 71개가 통과했다. 남은 실패는 원본 업데이트 완료와 읽기가 겹칠 때 `write.lock` 본문을 읽은 뒤 identity 검사 전에 정상 잠금이 사라지는 경쟁이다. 수정 전 실패를 재현하고, 실제 부재만 해제된 잠금으로 처리하며 남아 있는 파일의 오류는 보존했다. E2E도 원본·기준 이미지 저장 응답 이후 결과를 검증한다. 수정 코드의 저장 계약 79개·해당 E2E 1개·타입·필수 이름 712개(누락·중복·skip·only 0)를 통과했다. `6cf419d`의 [CI 34686633434](https://github.com/zzocojoa/storyboard-generator/actions/runs/34686633434)는 1,528개 중 1,527개가 통과했으며 기존 백업·복원·재편집 통합 검사 하나가 5초를 초과했다. 게시·바이트 복원·복원본 검토/편집을 독립 검사로 나누고 준비와 검사 각각의 5초 한도를 유지했다. 백업 11개·타입·필수 이름 714개 검사를 통과했다. 전체 원격 재검증은 진행 중이며 실패를 단순 재시도로 숨기지 않았다. 증거는 `.local/validation/automation-runtime/`의 `shared-audio-scope-ci-failed.log`, `released-lock-{red,focused,storage,e2e-fixed,types,registry,ci-failed}.log`, `backup-ci-{investigation,phased,types,registry}.log`다.
...
첫 여덟 구간은 revision 111까지 총 465초·76컷·133프레임으로 저장됐다. 원문·출처·전체 시간표, 다른 구간, 기존 자산·생성 기록 보존 및 구조 오류 0건을 각 저장 전후로 검사했다. 같은 원문의 그림·발화 공유는 실제 내레이션과 패널 구간에서 저장까지 확인했다. 프레임 그림과 가이드 음원은 아직 생성하지 않았으며 사람 승인 전이는 0회다. 긴 자막 2개는 필요한 읽기 시간과 최대 8초 설정이 충돌하고 손동작·발화 호흡·화면 밖 이동은 검토가 필요하다. 7번째 회상 구간의 이전 날 저녁 의상·공간과 8번째 패널의 발화 순서·고정 좌우 배치를 원문 및 앞선 제작 기준과 대조했다. 근거는 `first-eight-segments-verification.json`, 각 구간의 `*-segment-plan-verification.json`·`*-segment-editorial-review.json`이다. 실제 bitmap 연속성 검증은 아직 수행 전이다.
