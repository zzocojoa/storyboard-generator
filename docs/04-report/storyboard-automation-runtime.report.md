...
원격에서 확인한 수정 커밋 `feeae65de54c3ee542be9ac342bb69db41f5ad27`의 [CI 34680267391](https://github.com/zzocojoa/storyboard-generator/actions/runs/34680267391)는 `check`·`e2e` 모두 통과했다. 단위·통합 114개 파일·1,518개, 브라우저 E2E 71개, 실제 Audio 3회 반복 21개와 타입·Schema·필수 이름 701개(누락·중복·skip·only 0)를 확인했다. 이 커밋은 모델 보정 오류에 대상 자산·기대/현재 상태를 전달한다. 증거는 `.local/validation/automation-runtime/`의 `continuity-correction-ci-verification.json`, `continuity-correction-ci-full.log`, `continuity-correction-pr-state.json`이다. 아래 원문 공유 보완의 원격 검사와 구별한다.
...
첫 두 구간은 revision 104·105에 총 180초·27컷·51프레임으로 저장됐다. 원문 20개·글자 12개·발화/음향 지시 10개의 원문·출처와 전체 시간표, 다른 구간, 기존 자산·생성 기록 보존 및 구조 오류 0건을 독립 검사했다. 프레임 그림과 가이드 음원은 아직 생성하지 않았으며 사람 승인 전이는 0회다. 첫 구간 긴 자막의 읽기 요구 9,084ms와 최대 표시 설정 8,000ms가 충돌하고 녹음 청취 생략·손동작 가능성은 검토가 필요하다. 실제 화면에서 이 경고의 편집 버튼이 읽기 기준으로 이동하는지 확인했다. 오래 열린 이전 화면의 새 실행 종류 해석 오류는 전체 다시 불러오기 뒤 해소됐으며 서버 실행은 중단되지 않았다. `first-segment-plan-verification.json`, `second-segment-plan-verification.json`, `second-segment-editorial-review.json`, `revision-105-browser-review.json`이 근거다.

세 번째 내레이션 구간에서 같은 Unit을 그림과 발화에 함께 쓰는데도 음성 전용 Link를 요구하는 계약 충돌을 발견했다. 컷 안의 같은 Unit 중복은 금지되어 있으므로 유효한 단일 연결을 실패시키던 문제다. 음성 전용 Anchor가 있으면 기존 일치 검사를 유지하고, 직접 시각 Link만 공유할 때는 확정된 최초 공개 이후의 독립 Cue 시작을 허용하도록 수정했다. 미확정·누락·문맥 전용 근거, 조기 발화, 정보 하한, 중복 연결 차단을 유지한다. 실패 실행은 revision 105의 프로젝트를 전혀 변경하지 않았음을 확인한 뒤 기록 보존 취소했다. 사용량을 차감한 Run `b50f131d-d0f5-4cc3-bd17-c12297df39d3`은 앞선 두 구간을 건너뛰고 세 번째부터 실제 자동 제작 중이다. 새 서버와 화면은 Source Build `e65332abd98d70e582b5f8f42efc5aacf86db7e79c63a761b98615592ab707e1`을 사용한다. 재시작 당시 HEAD는 `feeae65`이고 미커밋 변경을 포함한 Build로 정확히 기록했다. `third-segment-failure-verification.json`, `continuation-visual-speech-request.json`, `continuation-visual-speech-start.json`, `visual-speech-server-status.json`이 근거다.

원문 공유 보완은 수정 전 실패를 재현했고 관련 3개 파일·44개 검사, 서버/Web 타입·Schema·필수 이름 703개(누락·중복·skip·only 0), 별도 웹 빌드를 통과했다. 전체 114개 파일·1,520개 중 1,519개가 통과했으며 남은 1개는 샌드박스의 localhost listen EPERM이다. 해당 Audio 계약 파일을 포트 허용 환경에서 동일 조건으로 실행하여 12개 모두 통과했다. 검사 시간·재시도 한도는 늘리지 않았다. 현재 보완의 원격 CI와 실제 내레이션 결과 적용은 아직 확인 전이다. 증거는 `visual-speech-contract-{red,focused,full-tests,audio-recheck,typecheck,web-typecheck,required-tests,schemas,web-build}.log`다. 이전 웹 출력은 `reference-retake-web-before-visual-speech`에 보존하며 배포 기록은 `visual-speech-web-publication.json`이다.
