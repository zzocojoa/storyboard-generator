...
전체 25분·12장면·24구간의 실제 콘티를 그림 373장·컷 231개로 생성하고, 사용자의 명시적 결과 확정에 따라 글자 시각 155개와 음향 지시 48개를 포함해 최종 승인했다. 원문·시간·그림·자산·생성 기록은 보존하고 승인 상태와 컷 잠금만 하나의 revision에 반영했다. 실제 파일·글꼴·공개 시각·시각 연결을 다시 검사하여 Final Ready, 차단 0개, 시각 연결 공백 0개를 확인했다. 가이드 음원 168개는 선택 기능으로 미생성 상태이며 기본 콘티 완료를 차단하지 않는다. 최종 상세 PDF·전체 그림 PDF·제작자용/구조화 CSV·현재 프로젝트 JSON과 등록 자산 446개를 로컬에서 출력했다. 그림 373개와 CSV 231행을 확인하고 출력 전후 저장본 및 모든 납품 파일의 SHA-256을 대조했다. 이 납품물은 과거 Version과 실행 기록 전체의 백업과 구분한다.

사용자는 기존 시각 검토의 품질 관찰 19묶음을 남긴 현재 결과를 그대로 승인했다. 해당 관찰을 수정 완료로 보고하지 않으며, 한 작품의 완료로 모든 작품의 시각 품질이나 장시간 실행의 무결함을 주장하지 않는다. 간헐적으로 관찰된 저장 잠금 잔류의 근본 원인은 아직 확정하지 않았다. 이번 실제 확정·출력의 근거는 로컬 실행 저장소 `diagnostics/final-confirmation/`과 `final/PRJ-008-r561/delivery-manifest.json`에 보존한다. 실제 제작 원문·그림·개인 실행 기록은 Git 전송 대상에서 제외한다.

## 기능별 검증 근거

자동 제작의 **생성 결과 불러와 검토**는 선택한 콘티·서버 상태·실제 자산 검사만 완료한 뒤 검토창을 연다. 검토에 필요하지 않은 전체 프로젝트 목록을 기다리지 않으며, 최신 콘티를 읽지 못하면 원인을 표시하고 이전 저장본의 검토창을 열지 않는다. 일반 새로고침의 전체 목록 갱신과 사람 승인·Final 검사는 유지한다. 수정 전 실제 브라우저 검사에서 최신 읽기 실패 뒤 검토창이 열리는 문제를 재현했다. 수정 후 같은 검사에서 실패 시 검토창 미표시와 전체 목록 응답을 보류해도 정상 검토가 가능한 경로를 확인했다. `npm run check`는 118개 파일·1,558개 검사(482.59초), 서버/Web 타입·필수 이름 743개(누락·중복·skip·only 0)·Schema·웹 빌드를 통과했다. 전체 E2E 74개(3.8분)와 실제 브라우저 오디오 3회 반복 21개(47.7초)도 통과했다. 근거는 `review-loading` 워크트리의 `.local/validation/review-loading/` 아래 `baseline-e2e.log`, `baseline-failure-trace.zip`, `fixed-e2e.log`, `check.log`, `e2e.log`, `audio-repeat.log`다. 이 수정은 자동 실행 저장소의 별도 잠금 오류를 해결한 것으로 보고하지 않는다.

전체 Version을 읽는 저장 감사에서 같은 생성 지시문 문자열을 공유하도록 수정했다. 각 revision의 객체·전체 metadata·이관 해시·변경 이력은 독립적으로 보존하며 파일을 다시 쓰지 않는다. 같은 ID의 변경·복원 지시문을 구별하고 반환 객체를 수정해도 다른 Version이나 Current에 전파되지 않는 회귀 검사를 추가했다. 저장 감사·이관·적용 일관성 165개와 자동 적용·실행·서비스 26개 검사, 서버 타입 검사, 필수 이름 743개(누락·중복·skip·only 0)를 통과했다. 메모리 제한을 높이거나 과거 이력을 생략하지 않는다. 전체 원본 보존과 메모리 측정의 로컬 검증 근거는 작업 워크트리 `.local/validation/history-memory/`에 둔다.

프로젝트 목록에서 컷별 시각 검사를 두 번 수행하던 부분을 하나의 평가로 통합했다. 같은 Snapshot의 안전한 컷 목록과 전체 Final 판정을 함께 계산하며 실제 자산 검사·차단 사유·승인 조건·응답 형식은 유지한다. 정상·반려 이미지·손상 이미지·시각 구간 누락·이전 화면 유지·검은 화면의 합성 사례 6개에서 이전 구현과 컷 ID 및 전체 판정의 일치를 확인했다. `npm run check`는 118개 파일·1,557개 검사(485.94초), 서버/Web 타입·필수 이름 743개(누락·중복·skip·only 0)·Schema 일치·웹 빌드를 통과했다. 전체 브라우저 검사 74개(4.1분)도 통과했고 기존 Vite 큰 청크 경고는 유지된다. 검증 근거는 작업 워크트리의 `.local/validation/readiness-summary-{check,equivalence,e2e}.log`다. 실행 중인 제작 서버는 이전 검증 버전을 유지한다.

같은 고정 컷의 연속 그림에서 구도·자세·소품 위치가 달라지는 실제 사례를 확인하여, 바로 앞 그림을 현재 입력과 대조한 뒤 첫 원본 참조로 전달하는 기능을 추가했다. 반려·미생성·다른 컷·뒤 그림·비활성 원문과 변경된 입력을 제외하고, 앞 그림이 참조한 이전 그림 사슬의 자산 교체·중간 프레임 삽입까지 검사한다. 기존 생성 1.0.0은 읽기만 하며 새 1.1.0 기록에 선택·제외 근거를 남긴다. 관련 15개 검사에서 실제 바이트 순서·해시·참조 한도·끝 프레임 시각·입력/사람 검토/이력 보존을 확인했다. 실제 작업본 revision 234의 6개 대상에서도 이전 생성 기록과 모든 선택 첨부 바이트의 호환성을 읽기 전용으로 확인했다. 새 기능의 실제 모델 호출·프로젝트 변경·사람 승인·진행 서버 배포는 아직 없으며 시각 품질 개선 자체를 완료로 판정하지 않는다. 근거는 작업 워크트리 `.local/validation/frame-continuity-{focused.log,actual-result.json,validation.json}`이다. 최초 전체 검사에서 로컬 포트 권한으로 중단된 1건은 제품 코드나 검사 제한을 변경하지 않고 실행 권한을 적용해 검증했다. 최종 `npm run check`는 118개 파일·1,557개 검사(485.49초), 서버/Web 타입·필수 이름 743개(누락·중복·skip·only 0)·Schema 일치·웹 빌드를 모두 통과했다. 초기 중단과 최종 결과는 `frame-continuity-check.log` 및 `frame-continuity-check-verified.log`에 보존한다.

발생별 근거가 없는 이전 자동 음향 초안이 재검토 대상에서 빠지는 문제를 수정했다. 원문 인용이 없거나 하나뿐이어도 미확정 전용 트랙은 모델의 발생별 검토로 이어지며, 실제 음원·수동 입력·사람 확인·보호 컷은 자동 변경하지 않는다. 실제 작업본 revision 210의 읽기 전용 검사에서 6개 구간의 이전 제안 7건을 발견했으며, 서로 다른 소리가 실제로 누락됐다고 확정한 건수와 구별한다. 전체 `npm run check`는 118개 파일·1,550개 검사(480.43초), 서버/Web 타입·필수 이름 736개(누락·중복·skip·only 0)·Schema 일치·웹 빌드를 통과했다. 브라우저 검사의 이전 모의 응답 3곳에 현재 모델 계약의 발생 근거를 추가했고, 발생별 화면에도 음원 준비와 배치 검토 상태를 구분했다. 최종 관련 E2E 4개(15.4초)와 타입·필수 이름·웹 빌드가 통과했다. 수정 전 실패·보호 데이터와 입력 바이트 보존 증거는 작업 워크트리의 `.local/validation/legacy-audio-occurrences-{red,focused-verified,check,e2e,e2e-verified,e2e-final}.log`, `legacy-audio-occurrences-actual-targets.json`과 각 실패 Trace다. 실행 중인 실제 제작 서버에는 아직 적용하지 않았고 실제 7건 재검토도 미실행이다.

가이드 음성 배정의 재시작 검사는 기존 편집 컷에서 실제 저장·Commit 직후 중단·재시작·같은 배정의 음성 호출을 확인한다. 로컬 계측에서 본래 목적과 무관한 제작 계획·기준 이미지 저장 작업이 함께 실행됨을 확인하여 기존 수동 컷 자료를 재사용하도록 정리했다. 배정 생성 1회·음성 호출 1회·그림 호출 0회와 원문·수동 컷 보존을 검사하며 제한 시간·재시도·제품 실행 코드는 변경하지 않는다. 수정한 `f2dc4b3`의 [원격 검사 34713756518](https://github.com/zzocojoa/storyboard-generator/actions/runs/34713756518)는 전체 1,550개 검사, E2E 74개 및 실제 브라우저 오디오 3회 반복 21개를 통과했다. 해당 재시작 검사는 원격에서 852ms였다. 근거는 `.local/validation/voice-casting-timeout/ci-result.json`과 `ci-complete.log`다. 이 원격 결과는 해당 Commit의 검증이며 이후 음향 초안 검토 수정까지 포함한 결과가 아니다.

미리보기의 **이 그림 편집**은 현재 표시된 시작·키·끝 그림의 편집 카드로 직접 이동하고 포커스 테두리를 표시한다. 여러 프레임이 있는 컷에서 컷만 열고 첫 카드부터 다시 찾던 문제를 재현한 뒤 수정했다. 데스크톱의 키 그림과 모바일의 끝 그림 이동·노출, 기존 재생·입력 보존을 포함한 브라우저 검사 5개(15.9초), 서버/Web 타입, 필수 이름 736개 검사와 웹 빌드가 통과했다. 프로젝트 원래 바이트와 미승인 그림의 Final 차단도 유지된다. 근거는 작업 워크트리의 `.local/validation/frame-target-focus/`와 `frame-target-{desktop,mobile}.png`다. 실행 중인 실제 제작 서버에는 아직 적용하지 않았다.
...
실제 자동 제작은 사용자가 승인한 누적 실행 시간 12시간·이미지 시도 700회·임시 미디어 1GiB 한도 안에서 진행한다. 확인한 revision 214는 24개 구간의 컷 계획을 모두 저장한 상태이며 전체 231컷·373프레임 중 그림 58개가 생성됐다. 계획된 373프레임의 실제 생성 입력과 참조 바이트·해시 검사를 통과했으며 기준 파일 204개·원문·기존 자산/생성 기록과 24개 구간 시간표를 보존했다. 구조 오류는 0건이고 사람 승인 전이는 없다. 전체 그림 생성·작품 검토·Final 출력은 남아 있으며 글자 가독성 6건을 포함한 제작자 검토 항목을 유지한다. 근거는 실제 작업 폴더의 `approved-12h-verification-revision-201.json`, `approved-12h-scene02-partial-visual-review-214.json`, `planned-frame-preflight-154.json`, `approved-12h-seg024-review.json`, `text-readability-review-proposal-154-verification.json`이다. 이 결과는 전체 자동 제작 완료나 Final 준비 완료를 뜻하지 않는다.

첫 장면 2분의 16컷·31그림을 직접 검토했다. 실제 브라우저에서 0초 고지, 15/18초와 32.9/33초의 메시지 공개, 79.9/80초의 대사 자막 공개를 확인했다. 각 문구는 지정 시점 전에 나타나지 않으며 원문 전체를 한 번 표시한다. 영상 아래의 별도 대사·음향 지시를 자막 중복으로 계산하지 않는다. revision 189의 119,998/119,999/120,000ms에서는 마지막 키 그림→끝 그림→다음 장면 그림으로 전환되는 실제 응답 바이트·해시와 화면을 확인했다. 고정 컷의 구도, 소품 종류·위치, 휴대전화를 든 손·가방 어깨의 연속성, 긴 자막의 읽기 시간은 구체적인 수정 후보로 남겼다. 컷 그림의 앞 프레임을 별도 구도 기준으로 첨부하는 입력은 현재 미구현이며 기준 자산·프레임 설명을 사용하는 재생성만 제공한다. 검토로 사람 승인을 변경하거나 재생성을 시작하지 않았다. 근거는 `approved-12h-opening-visual-review-187.json`, `opening-live-review/verification.json`, `reply-live-review/verification.json`, `voice-message-live-review/verification.json`, `first-scene-boundary-review/verification.json`이다.
...
입력 보존 보완 전 로컬 `npm run check`는 118개 파일·1,549개 검사(504.84초), 서버/Web 타입·필수 이름 735개(누락·중복·skip·only 0)·Schema 일치·웹 빌드를 통과했다. 수정 요청·이전 그림 전달·원문 보존의 관련 3개 파일·24개 검사도 통과했다. 전체 E2E는 74개 중 73개 통과, 글꼴 저장 검사 1개에서 현재본 게시와 잠금 정리 사이의 직접 저장소 읽기가 `PROJECT_BUSY`로 실패했다. 보존한 Trace와 코드 순서를 확인하고 저장 완료 HTTP 응답을 기다린 뒤 확인하도록 테스트를 수정했다. 관련 글꼴·재생성 브라우저 검사 3개(16.0초)와 최종 타입 검사를 통과했다. 제품의 저장 규칙·시간 제한·재시도 수를 변경하지 않았다. 증거는 작업 워크트리의 `.local/validation/reference-correction-{focused,check,e2e,repair-e2e,final-types}.log`와 첫 전체 브라우저 실패 Trace다. 수정 요청과 같은 소품 원형·수정 대상 이미지를 함께 보내고 실행 문구를 새로고침 뒤 다시 표시하는 경로를 확인했으며 실제 수정 요청의 생성 품질은 위 두 봉투 기준 범위에서 확인했다.

기준 이미지의 미전송 수정 요청과 소품 연결 입력은 대상 전환·새로고침 뒤 복원한다. 수정 전 실제 브라우저에서 대상 전환 후 문구가 빈칸으로 바뀌는 실패를 재현했다. 보완 후 대상별 입력 분리·복원, 한 번의 명시적 생성, 원문·이전 자산 보존, 생성 후 바뀐 revision의 재검토 차단을 확인했다. 사용되지 않거나 사라진 기준의 기록도 보관함에서 유지한다. 관련 단위 2파일·8개, 브라우저 3개(14.9초), 서버/Web 타입·웹 빌드·필수 이름 736개 검사를 통과했다. 실제 제작 서버는 기존 검증 Build로 계속 실행하며 이 UI 보완을 실행 중인 서버에 적용하지 않았다. 근거는 작업 워크트리의 `reference-retake-draft-{before-ui,unit,fixed-e2e,server-types,web-types,build,registry}.log`다. 첫 검증의 잘못된 웹 경로와 Chromium 샌드박스 기동 실패도 별도 로그로 보존하며 제품 재현 실패와 구별한다.
...
`parseProjectSnapshotEvidence`는 실제 원본·각 순방향 이관·검증된 Project의 감사 해시를 보존한다. 검증을 마친 전체 값이 마지막 이관 값과 구조적으로 동일한 경우에만 같은 JSON의 두 번째 직렬화·해시 계산을 생략한다. 모든 Version의 읽기와 전체 검증은 계속 수행하며 revision·Record ID·파일 시각으로 검증 결과를 재사용하지 않는다. 현재 형식·이전 형식·검토용 JSON 외피·생성 기록의 배열 순서 변경과 원본 보존을 검사했다.

실제 저장본 revision 0·293·358을 이전 코드와 수정 코드로 각각 3회 비교하여 Project와 모든 감사 해시의 일치를 확인했다. 약 36.1MB인 revision 358의 검사 중앙값은 452.3ms에서 259.0ms로 줄었다. 이는 한 저장본의 해당 함수 측정이며 전체 생성 시간이나 과거 작업 잠금 오류의 해결을 뜻하지 않는다. 원본 파일·승인·이미지는 변경하지 않았다. 비교 증거는 실행 자료의 `diagnostics/snapshot-hash-implementation-comparison.json`에 보존한다.

서버/Web 타입, 단위 검사 1,559개, Required Registry 744개, Schema 일치와 웹 빌드를 확인했다. 최초 전체 검사에서 로컬 수신 권한이 없는 `listen EPERM` 1건은 해당 시험 파일 12개를 권한이 허용된 환경에서 재검증해 통과했다. 검토·전달 브라우저 검사 4개를 확인했으며, 첫 시도의 배속 검사 한 건은 7,300ms에 정상 종료한 음원을 기본 polling 간격 때문에 종료 뒤 확인한 실패였다. 제한 시간과 검증 조건은 유지하고 2배속 확인 간격만 25ms로 바꾼 뒤 해당 검사가 통과했다. 최초 실패 로그·trace와 통과 로그는 수정 워크트리의 `.local/validation/snapshot-hash-dedup/`에 함께 보존한다. 장시간 자동 실행의 잠금 오류 원인과 작품 전체 품질 검증은 계속 남아 있다.


생성 기록 비교에서는 양쪽의 전체 `GenerationSchema`를 검증한 뒤 지시문 문자열을 직접 비교하고 나머지 전체 metadata만 기존 JSON 동등성으로 판정한다. 긴 지시문의 반복 직렬화를 생략하면서 숫자 `-0`/`0`의 기존 취급도 유지한다. 변경 기록·배열 순서·중첩 Build metadata는 계속 검출하며 전체 Version 읽기·원문·참조·최초 도입·삭제·재등장 검사를 줄이지 않는다.

실제 저장본 392의 전체 393개 Version을 같은 입력으로 검사한 결과, 생성 기록 감사 함수는 43.3초에서 4.9초로 줄었고 감사 결과 전체가 일치했다. 파일을 수정하지 않았으며 현재 제작 서버는 이전 코드로 계속 실행했다. 실제 최근 두 그림의 생성·준비는 각각 107.3초/105.0초, 저장 반영은 263.2초/260.1초였으나 이번 함수 측정을 전체 저장 반영 시간의 개선으로 해석하지 않는다. 비교 증거는 `.local/validation/record-audit-comparison.json`과 실행 자료의 `diagnostics/frame-state-progression/measured-first-two-frame-timing.json`에 보존한다.

현재 구현은 표준 문자열·JSON 연산만 사용하며 서버/Web 타입을 통과했다. 전체 118파일·1,565개 검사 중 1,564개가 통과했고(462.00초), 로컬 수신 권한 `listen EPERM`으로 실패한 오디오 진단 파일은 허용된 환경에서 12개 모두 통과했다(1.01초). Required Registry 750개(누락·중복·skip·only 0), Schema 일치와 Web 빌드도 확인했다. 최초 Node 비교 함수의 브라우저 호환 실패 로그와 권한 실패·재검증 로그는 수정 워크트리의 `.local/validation/record-equality-*.log`에 보존한다. 실제 서버 전환과 전체 반영 시간 개선은 아직 검증하지 않았다.

## 자동 음향 배치의 실행 간 보존

새 자동 실행이 사람 검토 대기인 음향 발생을 아직 배치하지 않은 것으로 판단해 이미 완료한 8개 구간의 배치를 다시 요청했다. 현재는 최근 구간 계획·연결 보완 기록의 입력과 결과를 원문·정보 조건·발생 연결·Cue 시각에 대조하고 현재 콘티 출력 조건을 재검사한다. 유효한 배치는 새 실행에서도 보존하며 사람 검토 상태와 실제 음원 부재는 그대로 유지한다. 변경된 원문·배치·Source Anchor, 근거 없는 이전 기록은 보완 대상으로 남기고 손상 기록은 명시적 오류로 처리한다. 저장 형식이나 기존 생성 기록을 변경하지 않는다.

실제 revision 363의 읽기 전용 사전 검사에서 이전 코드는 8개 연결 보완 작업을, 수정 코드는 아직 이미지가 없는 197개 프레임만 선택했다. 원본 파일 바이트·Project·승인 상태를 보존했다. 근거는 실행 자료의 `diagnostics/planned-sound-timing-preflight.json`이다. 이 검사는 새 실행의 작업 선택이며 실제 서버 전환이나 전체 제작 완료를 뜻하지 않는다.

관련 5파일·30개 검사를 통과한 뒤 `npm run check` 전체가 한 번에 통과했다. 118파일·1,559개 검사(441.69초), 서버/Web 타입, Required Registry 744개, Schema 일치와 웹 빌드를 확인했다. 음향 발생의 원문·시각 표시·명시적 확인, 자동 제작·중단 재개·기존 편집과 실제 WAV 보존 브라우저 검사 11개도 통과했다(34.7초). 로그는 수정 워크트리 `.local/validation/planned-sound-timing/{check,e2e}.log`에 보존한다. 실제 작품 전체 품질·사용자 승인과 장시간 잠금 오류의 원인 확인은 별도로 남아 있다.

## 중간 그림의 시작 상태와 앞선 동작 보존

중간 프레임에서 컷 시작의 조명·공간·소품 상태가 입력에서 사라지는 문제를 보완한 뒤, 실제 후속 생성에서는 이미 공개한 소품을 시작의 화면 밖 상태로 되돌리는 문제가 확인됐다. 현재는 시작 시각·시작 상태 → 현재도 유효한 직접 원문 안의 앞선 프레임 연출 → 현재 프레임 순으로 상태를 적용한다. 같은 컷의 앞선 연출만 실제 평가 시각 순서로 전달하며, 이미 공개·이동·조작한 상태는 현재 설명에서 반복하지 않아도 유지한다. 현재의 명시적 변화가 우선이고 미래 프레임·다른 컷·미확정 또는 현재 비활성 원문은 제외한다. 연출 제안을 사람 승인이나 실제 그림의 검증 증거로 취급하지 않는다.

신규 그림 기록은 `automatic-frame-image-1.3.0`에 앞선 연출과 입력 해시를 보존한다. 이전 1.0.0·1.1.0·1.2.0 기록은 당시 전달한 필드만으로 대조하며 과거 입력·자산·승인을 재작성하지 않는다. 앞선 연출이 바뀌면 그것을 참조한 새 그림의 의미 입력도 달라진다. 기존 앞 그림의 바이트·참조 사슬 검사는 그대로 유지한다.

실제 저장본 390의 두 중간 프레임을 수정 전 코드와 읽기 전용으로 비교했다. 앞서 소품을 공개한 연출과 이어서 휴대전화를 꺼낸 연출이 시각 순서로 전달되며 현재 원문·시작 상태·원본 파일 바이트가 유지됐다. 미래 프레임은 포함되지 않았다. 기존 그림 203장은 보존했고 자동 실행은 정상 일시 중단했다. 이 비교는 생성 입력 검증이며 수정본으로 새로 생성한 그림의 품질이나 전체 작품의 완성도를 증명하지 않는다.

관련 프레임 검사 20개와 전체 `npm run check`를 통과했다. 전체 118파일·1,564개 검사(444.99초), 서버/Web 타입, Required Registry 749개(누락·중복·skip·only 0), Schema 일치와 웹 빌드를 확인했다. 합성 자료로 앞선 공개·조작의 순서, 현재/미래/다른 컷·비활성 원문 제외, 미확정 연결 차단, 입력 변경과 이전 기록 호환을 검사했다. 근거는 수정 워크트리의 `.local/validation/progression-{red,focused,check}.log`, 실제 저장본 비교는 `frame-progression-reproduction.json`에 보존한다. 새 실제 이미지 생성과 전체 작품의 시각 검토·최종 출력 검증은 남아 있다.
