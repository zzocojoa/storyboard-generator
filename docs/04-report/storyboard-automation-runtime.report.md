...
수정 커밋 `8b15a88682c07888156ca595f12d1f209b1fdc89`의 [CI 실행 34674892415](https://github.com/zzocojoa/storyboard-generator/actions/runs/34674892415)에서 필수 `check`·`e2e`가 모두 통과했다. 단위·통합 검사 113개 파일·1,512개, 필수 이름 696개(누락·중복·skip·only 0), 서버/Web 타입·Schema·웹 빌드, 전체 브라우저 E2E 70개와 실제 오디오 3회 반복 21개를 확인했다. 글자 배치 HTTP fixture는 로컬 `dist/web` 의존을 제거하고 테스트별 임시 웹 폴더와 HTML을 사용한다. 실행 서버의 `dist/web`를 건드리지 않은 Git archive 기반 체크아웃에서도 해당 7개 검사와 타입 검사를 통과했다. 제품 출력 조건·검사 수·시간 제한은 동일하다. 최초 CI의 7개 실패 로그와 원인 검증은 `.local/validation/automation-runtime/`의 `pr-ci-failure.log`, `pr-ci-clean-text-layout.log`, `pr-ci-fixture-types.log`에, 원격 성공 로그와 PR 해시·검사 상태는 `pr-ci-fixed-success.log`, `pr-final-state.json`에 보존한다. 초안 PR의 통과를 실제 작품 전체 제작·품질 검증이나 최종 머지로 해석하지 않는다.

실제 PRJ-008 전체 실행 `cb97bf3b-ad21-4d9b-806f-0ef6d12f44fd`은 revision 93에서 공간 연속성 결함을 확인하여 중지·기록 보존 취소했다. 24개 구간의 제작 기준·Resource 66개 중 기준 이미지 59개, 등록 작업 77개 중 완료 70개, 이미지 시도 60회, 활성 시간 6,898,960ms를 기록했다. 같은 집의 4개 기준 그림을 직접 비교했을 때 찬장 종류·식탁·창문·주방 위치가 달라졌고, 세 변형의 생성 입력에 앞선 기본 공간 이미지가 없었다. 인물은 동일 외형 참조가 동작했으나 장소에는 적용하지 않던 구현 결함이다. 이전 revision 55의 21개 실제 미디어 해시·디코딩·원문 보존 검사 결과는 그대로 보존한다. `reference-stage-verification.json`, `character-reference-continuity-review.json`, `location-continuity-defect.json`, `location-review-cancelled.json`이 근거다. 이 상태를 작품 전체 그림·연출·Final 승인 완료로 보고하지 않는다.

원본 장소 ID가 같은 앞선 기본 공간 이미지를 실제 생성 입력으로 연결했고, 기준 한 장을 선택하여 재생성·비교하는 API와 제작 설정 화면을 추가했다. 사건 상태가 있는 기본 후보·다른 장소·독립 세트·소품 간 추측 연결은 제외한다. 실행·저장·기존 이미지 보존과 선택 범위 검증은 기존 자동 제작 계약을 사용하며 사람 승인을 만들지 않는다. 관련 제작 기준 검사 14개, 재생성 검사 3개 및 선택 재생성 브라우저 E2E 1개가 통과했다. 최초 새 HTTP 검사의 전체 자동 제작 fixture가 5초 한도를 넘은 경우는 합성 기준을 한 저장 후보로 준비하도록 바꾸었고 제품 또는 테스트 시간 제한은 늘리지 않았다. 서버/Web 타입 검사와 대체 웹 출력 빌드를 통과했다. 전체 회귀 114개 파일·1,517개 검사 중 1,516개가 통과했고 한 검사는 샌드박스의 localhost listen EPERM으로 실패했다. 해당 Audio 계약 파일을 포트 허용 환경에서 재실행하여 12개 모두 통과했다. 필수 이름 700개(누락·중복·skip·only 0)와 Schema 일치 검사도 통과했다. `location-continuity-unit.log`, `reference-retake-focused-recheck.log`, `reference-retake-e2e.log`, `reference-retake-audio-sandbox-recheck.log`, `reference-retake-registry.log`, `reference-retake-schema-check.log`와 모바일 비교 화면이 근거다. 추가 구현 커밋 `7c44c00c3d839803249415b541eb4b7cd8399075`의 [CI 34677681953](https://github.com/zzocojoa/storyboard-generator/actions/runs/34677681953)에서 필수 `check`·`e2e`가 모두 통과했다. 단위·통합 114개 파일·1,517개, 전체 브라우저 E2E 71개(4.6분), 실제 Audio 반복 21개(55.5초)와 타입·Schema·필수 이름·웹 빌드를 확인했다. PR #8은 같은 HEAD의 열린 초안이며 `MERGEABLE`·`CLEAN`이다. `reference-retake-ci-success.log`, `reference-retake-pr-state.json`이 근거다.

수정된 전용 서버 `127.0.0.1:57847`에서 실제 공간 변형 세 장을 각각 1회 생성하여 revision 93→96에 보존했다. 총 활성 시간 240,227ms이며 세 결과 모두 기본 공간의 찬장·식탁·의자·창문·주방 출입구·벽·문 구조를 유지하는지 직접 비교했다. Dataset·Sources·Handoff·컷·프레임·글자·음성 Cue·이전 자산·기록은 동일하고 전체 미디어 62개의 실제 SHA-256이 일치했다. 사람 승인 전이는 0회다. 누적 사용한 시간·작업·이미지·바이트를 남은 예산에서 제외하고 Run `c8aca7cf-f99c-41ec-8bf1-0e659348fe87`으로 24개 구간의 남은 자동 제작을 이어간다. 이어진 실행은 revision 103에서 24개 구간의 기준 Resource 66개 모두에 이미지를 준비했다. 현재·이전 기준 이미지 69개의 해시·디코딩과 원문·사람 승인 보존을 검사했고, 마지막 7개 결과와 두 인물의 앞선 기준을 직접 비교했다. 첫 구간의 컷·대사·글자·음향 배치는 revision 104에 저장됐으며 다음 구간을 계획 중이다. 전체 컷·그림 생성과 작품 품질 검토는 남아 있다. `reference-stage-complete-verification.json`, `reference-stage-final-seven-visual-review.json`이 근거다. 근거 파일은 `.local/validation/automation-runtime/prj008-whole-story-20260912/`의 `location-repair-verification.json`, `location-repair-evening-verification.json`, `location-repair-night-verification.json`, `continuation-after-reference-repair.json`이다.

현재 기준 이미지 66장의 실제 시각 검토 기록을 모두 대조했다. 인물·공간 22장과 종이·봉투 소품 6장, 나머지 소품 27장의 관찰을 앞선 공간 변형 4장과 마지막 생성 7장의 기록에 합쳐 누락이 없음을 확인했다. 인물 변형은 앞선 얼굴·머리와 설정별 의상을 비교했다. 봉투 두 장은 요청한 앞면 대신 뒤쪽 입구가 보이고 기본형과 찢어진 상태의 덮개 모양이 달라 검토 대상으로 남는다. 명시적 공통 대상 ID가 없는 두 소품의 기준 관계를 자동 확정하지 않았다. `reference-stage-expanded-visual-review.json`, `reference-stage-remaining-props-visual-review.json`, `reference-stage-complete-visual-coverage.json`에 실제 파일 해시·자원·자산 ID·관찰을 보존했다. Project 쓰기와 사람 승인 전이는 0회다.

첫 구간은 설정된 모델 보정 2회 안에서 지정 장소·연결 중복·인접 컷 상태 오류를 해소하여 120초·16컷·31프레임의 후보를 revision 104에 저장했다. 별도 스냅샷 검사에서 원문 10개와 글자 5개, 발화·음향 지시 3개의 원문·출처, 전체 입력 시간표, 다른 구간, 기존 기준 이미지·생성 기록 보존과 구조 오류 0건을 확인했다. 실제 프레임 그림은 아직 없고 가이드 음원도 생성하지 않았다. 독백 20초·녹음 발화 25초의 정적 구도는 원문에 따른 연출 이유를 남겼다. 긴 자막 한 개는 읽기 요구 9,084ms와 최대 표시 설정 8,000ms가 충돌하며, 녹음 청취의 생략 편집·손동작 가능성은 검토가 필요하다. 원문 축약이나 사람 승인을 적용하지 않았다. 근거는 `first-segment-plan-project.json`, `first-segment-plan-verification.json`이다.

자동 보정에 전달하던 오류 문구에서 비교한 기대·현재 상태와 연속성 자산 식별자가 빠진 문제를 수정했다. 모델은 실제 오류 대상과 앞 컷의 종료 상태를 보고 후보를 보정할 수 있고, 프롬프트는 지정 장소와 같은 컷의 원문 연결 단일성·인접 상태 기록 규칙을 설명한다. 검증 규칙과 보정 횟수는 바꾸지 않았다. 관련 5개 파일·46개 검사가 통과했으며 서버/Web 타입·Schema·필수 이름 701개(누락·중복·skip·only 0)를 확인했다. 전체 회귀 114개 파일·1,518개도 479.21초에 모두 통과했다. 별도 출력 폴더의 웹 빌드가 통과했고 기존 큰 청크 경고는 유지한다. 기존 기준을 유지하며 실제 작품 실행은 재시작하지 않았다. `continuity-correction-focused-final.log`, `continuity-correction-full-tests.log`, `continuity-correction-schemas.log`, `continuity-correction-web-build.log`와 `continuity-correction-validation.json`이 근거다. 이 보완의 원격 CI는 아직 확인 전이며 앞서 통과한 CI와 구별한다.
...
- 원격 master의 PR 필수·strict check/e2e·관리자 적용·강제 푸시/삭제 금지는 유지되고 있다. 원격 `0d0525ab1c8a6cedaf79d599068314329bdcbb3a`의 check/e2e는 성공이다. **현재 codex/document-handoff 구현에는 아직 PR이 없으며 이 원격 성공을 현재 변경의 CI 성공으로 취급하지 않는다.** 증거: `text-presentation-remote-audit.json`, `text-presentation-master-checks.json`.
