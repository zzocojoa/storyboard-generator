...
가이드 음성 배정의 재시작 검사는 기존 편집 컷에서 실제 저장·Commit 직후 중단·재시작·같은 배정의 음성 호출을 확인한다. `a90f778`의 [원격 검사 34712767461](https://github.com/zzocojoa/storyboard-generator/actions/runs/34712767461)는 이 검사 한 건의 5초 시간 초과로 실패했고 E2E는 실행되지 않았다. 로컬 계측에서 본래 목적과 무관한 제작 계획·기준 이미지 저장 작업이 함께 실행됨을 확인하여 기존 수동 컷 자료를 재사용하도록 정리했다. 배정 생성 1회·음성 호출 1회·그림 호출 0회와 원문·수동 컷 보존을 검사하며 제한 시간·재시도·제품 실행 코드는 변경하지 않는다. 관련 3개 파일·18개 검사(24.56초), 서버 타입과 필수 이름 736개(누락·중복·skip·only 0)가 통과했고 해당 검사는 2,586ms였다. 수정 전 계측 3,363ms는 로컬 관측이며 원격 지연 원인 전체를 확정하는 수치는 아니다. 근거는 `.local/validation/voice-casting-timeout/`이다. 수정 후 원격 전체 검사 결과는 아직 확인 전이다.
