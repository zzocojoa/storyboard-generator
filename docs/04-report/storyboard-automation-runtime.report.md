...
직접 원문이 바뀌어 앞 그림 참조가 제외되는 중간 프레임에서 컷 시작의 조명·공간·소품 상태도 입력에서 사라지는 문제를 확인했다. 현재는 시작 시각과 현재 화면 기준 대상의 시작 상태를 중간 프레임에도 전달하며 현재 원문·명시적 동작을 우선한다. 종료 상태·화면 밖 대상과 유효하지 않은 앞 그림은 계속 제외한다. 이전 1.0.0·1.1.0 기록은 당시 입력 범위로 대조하고 신규 1.2.0 기록에만 시작 시각을 보존한다.

합성 자료로 상태 전달·미래/화면 밖 상태 제외·원문 전환·기존 기록 호환·변경된 시작 기준의 무효화를 검사했다. 실제 저장본 revision 388의 같은 프레임 입력을 이전 코드와 비교하여 시작 상태 0개에서 6개 전달로 바뀌고 활성 원문·앞 그림 제외·원본 바이트는 유지됨을 확인했다. 실제 이미지 개선 효과와 전체 작품의 시각 완성도는 아직 검증하지 않았다.

관련 프레임 검사 17개, 서버/Web 타입 검사를 통과했다. 전체 118파일·1,561개 검사에서 1,560개 통과 후 로컬 포트 수신 권한 오류 1개가 발생했고, 해당 시험 파일 12개를 권한이 허용된 환경에서 재검증해 모두 통과했다. Required Registry·Schema 일치·웹 빌드도 통과했다. 최초 실패와 재검증 기록은 수정 워크트리의 `.local/validation/{check,audio-socket-permission,check-remaining}.log`, 실제 저장본 비교는 `frame-state-reproduction.json`에 보존한다. 기존 그림·원문·승인·누적 실행 한도는 변경하지 않았다.
