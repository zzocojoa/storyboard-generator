...
상태: 1.5.0 도메인 계약, 독립 Placement 정보 판정, 실제 미디어 무결성·자원 검사, backpressure가 적용된 Worker WAV 정규화, 중앙 Asset reference closure, symlink-safe 파일 연산, file identity와 전체 참조 분석을 사용하는 journal version 3 저장·Initial Project 복구, Create 소유 lock에 의한 Create–Update 직렬화, 프로젝트별 recovery block, Cue 범위 Audio 수명주기, 안전 Frame·Audio 출력, 명시적 J/L-cut과 PRJ-007 실제 UNIT-045 WAV Golden을 반영한 현재 설계. 전체 분량의 제작 판단은 별도 검토가 필요하다.
...
| GET /api/status, /api/codex/requests/:id | 영속 생성 요청 지표·오류·결과 revision, 시작 시 저장 복구 결과와 `storageRecoveryBlocks` |
...
Generation Record도 revision 사이에서 append-only다. 기존 배열 항목의 삭제·재정렬·삽입과 provider·model·prompt·resultAssetIds·shotIds·createdAt을 포함한 전체 metadata 변경을 거부한다. 신규 Record는 배열 끝에만 추가할 수 있다. 각 `shotIds`는 Next Project의 실제 Shot을, 각 `resultAssetIds`는 Next catalog의 실제 Asset을 가리켜야 하므로 같은 revision에서 새 Shot·Asset·Record를 함께 추가할 수 있다. Source Update와 Asset 교체도 과거 Record를 바꾸지 않는다.
...
Initial Create는 Asset metadata와 `frames.imageAssetId`, `audioCues.assetId`, `generationRecords.resultAssetIds`, `shots.propIds`, Shot의 두 continuity Asset 참조가 모두 없는 Project만 지원한다. 이 검사는 schema 형태 확인 직후, data root 초기화와 staging·journal·final 생성 전에 실행한다. 위반 시 Project ID, entity, Asset ID, 기대·실제 kind와 subject, metadata·참조 수, 필드 목록과 update 등록 방법을 담은 `UNSUPPORTED_INITIAL_PROJECT_ASSETS`를 반환한다. 허용된 create는 초기화와 transaction ID 생성 뒤 `<dataRoot>/.create-locks/<sha256(projectId)>.lock`을 `O_EXCL`로 먼저 획득한다. root lock 아래에서 final 존재를 다시 확인하고 staging·journal을 만든다. staging의 final `write.lock`은 root lock과 같은 Project ID·transaction ID·host·PID를 가지며 Current·Version 0·빈 관리 디렉터리·Asset-free 상태를 검사한 뒤 final과 함께 게시된다. final 검증과 create journal cleanup을 마치면 final lock을 먼저 제거하고 같은 metadata와 inode인 root lock을 마지막으로 제거한다. 이 동안 같은 Project의 다른 Create와 Update는 `PROJECT_BUSY`이며 transform과 version 1, 추가 transaction을 만들지 않는다. 다른 Project는 독립 root lock을 사용한다.

Create recovery는 `.create-locks`의 root lock을 먼저 읽는다. 같은 Host의 살아 있는 owner는 해당 Project만 Active Create로 기록하고 다른 Project의 초기화·읽기·변경은 계속한다. 종료된 owner는 matching v3 journal과 final lock의 Project ID·transaction ID·host·PID를 확인한 뒤 commit 또는 rollback을 끝낸다. journal 전 중단은 root lock만 제거하고, 완전한 final은 journal 유무와 관계없이 검증한 뒤 남은 lock을 제거한다. root lock·journal·final lock이 불일치하거나 다른 Host·손상·불완전 상태이면 root lock과 관련 자료를 보존하고 해당 Project에 `STORE_CREATE_RECOVERY_REQUIRED` marker를 남긴다. journal v2와 기존 Asset-bearing 저장본의 읽기·복구 정책은 유지한다.
...
복구 실패는 `.recovery-blocks`의 프로젝트별 marker에 저장한다. 같은 instance와 다음 process의 mutation은 `STORE_RECOVERY_BLOCKED`로 거부하고 read-only와 안전 출력은 current를 읽을 수 있을 때 유지한다. `/api/status.storageRecoveryBlocks`가 원인과 transaction을 표시한다. 재시작에서 복구 증명이 성공한 경우에만 marker와 lock을 지운다. 복구 결과는 구조화 로그와 `/api/status.storageRecovery`에도 기록한다.

HTTP 오류 응답은 기존 `code`, `message`, `issues`에 `category`, `retryable`, `operatorActionRequired`를 추가한다. Validation은 400, Not Found는 404, Busy·Revision Conflict·Already Exists는 409, 저장 복구·정리 개입은 423, 일시적인 lock 획득 실패는 503, 분류되지 않은 오류는 500이다. Web은 Import 요청을 진행 중 한 번만 제출하고 `IMPORTING`을 표시한다. 423에서는 **STORAGE RECOVERY REQUIRED** 배너와 mutation 잠금을 유지하며 자동 재시도하지 않고, 503은 일시 장애로 표시한다.
...
6. 두 가지 이상의 구성으로 회귀·브라우저 검증, 전체 요구사항 감사: 28개 파일의 666개 자동 테스트로 합성 자료와 초기 회귀 자료의 가져오기·편집·출력을 검증했다. 정확한 이름의 계약 109개는 Asset reference 여섯 필드, Initial Create의 disk side effect 없는 거부, Project별 Root Create Lock과 두 Store의 동시 Create, crash recovery, Project 단위 Active Create, Generation Record append-only, HTTP 400·404·409·423·503·500과 Web UI 상태, 기존 저장·journal v2/v3·Worker queue, PRJ-007 Golden을 포함한다. 기존의 동일 hash·다른 inode, 모든 version·다른 transaction 참조, symlink, 영속 recovery block과 큰 PCM24 업로드 중 status 응답도 유지한다. PRJ-007 Golden은 실제 48,000Hz 2초 WAV를 `UNIT-045`의 849,000–851,000ms J-cut에 연결하고 Generation Record 불변성도 확인한다. 전체 분량의 시각·낭독 검토는 남아 있다.
