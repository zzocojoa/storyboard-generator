...
- `ShotSourceLink`는 컷과 원문 Unit의 권한 관계다. `primary-visual`, `continued-visual`, `audio-only`, `context-only` 용도와 `confirmed`, `mapping-required` 상태, `shot-offset`·`frame-range`·`frame`·`unresolved` 시간 Anchor를 가진다. `SOUND`와 `MUSIC`은 직접 시각 근거가 될 수 없고 `continued-visual`은 앞선 `primary-visual`을 요구한다. `sourceUnitIds`는 1.9.0 프로젝트에 중복 저장하지 않는다.
...
Visual Plan은 ShotVisualPlanInputSchema의 Mode와 전체 Source Links를 동시에 적용해 단일 다음 Project를 검사한다. Lock·중복·종류·continued 근거·Anchor·시간 순서·Coverage·Black/Hold·Gate를 검증한다. `/visual-plan`은 한 revision만 추가하고 proposed/pending으로 무효화하되 기존 Asset·Record를 보존한다. UI는 하나의 Draft·Coverage Preview·Issue·저장 버튼을 사용한다. Preview와 저장은 `reviewVisualPlanChangeIssues`를 공유한다. 변경 대상 Shot-local 오류는 항상 차단한다. Segment 오류는 code·severity·entityId·field·expected·actual을 비교해 새 오류와 악화만 차단하고, 동일하거나 최초 공개 역전 간격이 엄격히 줄어든 다른 Shot 오류는 `existingUnrelatedIssues`로 표시한다. Legacy Shot을 순차 수리할 수 있지만 Approval·Final은 기존 오류도 계속 차단한다. 기존 Content의 Mode 단독 변경은 VISUAL_PLAN_ATOMIC_UPDATE_REQUIRED다.
...
Schema 1.6→1.7은 과거 Record의 generatorBuild=null을 보존하고 1.7→1.8은 non-null Legacy Build의 신규 hash·dirty 상태를 null로 추가하며 Transition 노출 의미를 결정적으로 이관한다. 1.8→1.9는 `migrate18To19`과 `migrateGeneratorBuildInput`으로 과거 Build의 availability를 null로 추가하고 기존 dirty 값·Source·ID·시간·Anchor·Asset·Prompt를 보존한다. 최신 입력 멱등성과 전체 Migration 체인을 검사하고 Project·Request 읽기에서 원본·Version 파일은 재작성하지 않는다.

Build Manifest provenanceVersion 3의 headCommitSha는 실제 HEAD이며 worktreeDirty와 generationInputsDirty는 독립 상태다. 현재 Manifest의 gitStateAvailable은 boolean, 영속 과거 Provenance에서는 boolean|null이다. rev-parse·status 모두 성공해야 true이며 status 실패는 확인된 HEAD와 false·dirty=null, rev-parse 실패는 HEAD=null·false·dirty=null이다. 과거 availability 없는 Build는 null이고 기존 dirty 값은 재해석하지 않는다. 이 감사 필드만으로 sameGenerationBuild가 달라지지 않는다. commitSha는 deprecated HEAD alias다. sourceTreeSha256은 Runtime Source, generationContractSha256은 Skill·AGENTS·Codex·Proposal·관련 Domain/Prompt/JSON Schema·package 계약, runtimeGenerationConfigSha256은 허용된 음성·Provider·Audio 출력 설정을 묶는다. 상대경로 정규화·정렬·NUL 구분과 Stable JSON을 적용하고 Secret·절대경로·PID·Host·builtAt은 Fingerprint에서 제외한다. 같은 Target/Basis와 세 hash·Schema가 같은 Pending만 재사용한다. 이전 Build Pending은 superseded로 보존하고 새 요청을 만든다. Metrics는 superseded를 실패율에서 제외하며 Context·Apply도 Build를 검증한다.
...
Summary Integrity Cache는 Project ID·불변 Asset 전체 metadata·file dev/ino/size/mtime/ctime와 Audio의 프로젝트 sampleRate를 키로 최대 1,024개 LRU를 유지한다. revision은 키에 포함하지 않으며 비Asset 수정·catalog append는 기존 항목을 보존한다. 검사 전후 identity·metadata를 비교해 최초 포함 최대 2회만 다시 검사한다. 계속 변하면 `STORED_ASSET_CHANGED_DURING_CHECK`로 비검증 처리하고 캐시·finalOutputReady·안전 수치에서 제외한다. 파일 교체·크기·mtime·ctime·Asset 변경은 해당 항목을 무효화하며 close에서는 전부 비운다. Final Readiness·Final Export·Bundle·Safe Visual/Frame/Audio·다운로드·생성 Reference에는 이 캐시를 사용하지 않는다.
...
### Request 전이·Lock 게시·Bundle 출력 소유권
...
별도 `audio-stress.yml`은 Ubuntu·Node 24·Chromium에서 해당 Workflow 파일을 변경한 pull_request의 50회 검증을 지원한다. workflow_dispatch 50/75/100회와 주간 schedule 기본 50회는 기본 Branch에 Workflow가 등록된 뒤 사용할 수 있다. 일반 PR의 3회 반복·기본 Timeout·Retry 0은 유지한다. 명시적 bash pipefail과 max-failures=1로 첫 실패를 Job 실패로 보존한다. Custom Reporter는 완결된 성공/실패/미완 반복과 첫 실패 번호·테스트, retry·예상 밖 Heartbeat 오류 수를 JSON에 기록한다. 실패 시 test-results와 console log를 Artifact로 올리며 Trace는 retain-on-failure다.

재생 경과 시간은 `max(0, frameTimestamp - startedAt)`으로 계산한다. 첫 RAF 시각이 effect의 시작 시각보다 이른 경우에도 Playhead를 Cue 시작 전으로 되돌리지 않는다. 별도 RAF 경계 회귀는 첫 callback 시각만 제어하며 Audio는 실제 HTMLAudioElement를 유지한다. 각 Case는 실제 HTMLAudioElement cold load 이후 별도 Full 200·Range 206을 검증한다. 서버 request/send/finish/close/aborted/socket-error의 시각·route·method·Range 존재·status·byte 수·elapsed와 App/Store/Worker close를 기록한다. 브라우저 metadata/canplay/playing/seek/pause/ended/error·ready/networkState·currentTime/duration/src·요소 수·UI 오류·HTTP Content-Range를 함께 보존한다. Body bytes·사설 Header는 진단 로그에서 제외한다. Context와 연결 풀 → App/Store/Worker → 소유 Socket/Listener/Timer 종료를 확인한 뒤 임시 Root를 삭제한다. 남은 수치는 이 시험이 소유한 자원의 범위이며 프로세스 전체의 내부 런타임 handle 수를 0으로 주장하지 않는다. 반복 성공은 이전 Linux 간헐 실패의 원인 해결 증거가 아니다.

## 8. 검증 계획과 구현 순서

1. 버전 고정 패키지, 1.9.0 스키마·타입, 1.0.0→1.1.0→1.2.0→1.3.0→1.4.0→1.5.0→1.6.0→1.7.0→1.8.0→1.9.0 Migration, 파일 해시·경로·권한 검사, native 입력: 구현 및 자동 검증됨.
...
6. 두 가지 이상의 구성으로 회귀·브라우저 검증, 전체 요구사항 감사: 단위·통합 테스트와 실제 Chromium E2E로 합성 자료와 초기 회귀 자료의 가져오기·편집·출력을 검증한다. Required Registry의 정확한 이름의 필수 계약은 Proposal Frame·Visual Mode, 주기 Heartbeat, Historical Audit, Marker Quarantine, Asset Integrity, 열린 Placement, Timecode, Status Refresh와 브라우저 흐름을 포함한다. 격리된 실제 HTTP smoke는 동적 포트에서 정상 출력과 409·423·503을 검증하고 모든 임시 자원을 정리한다. PRJ-007 Golden은 실제 48,000Hz 2초 WAV를 `UNIT-045`의 849,000–851,000ms J-cut에 연결하고 Generation Record 불변성도 확인한다. 전체 분량의 시각·낭독 검토는 남아 있다.
