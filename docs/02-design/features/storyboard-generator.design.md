...
Initial Create는 Asset metadata와 `frames.imageAssetId`, `audioCues.assetId`, `generationRecords.resultAssetIds`, `shots.propIds`, Shot의 두 continuity Asset 참조가 모두 없는 Project만 지원한다. 이 검사는 schema 형태 확인 직후, data root 초기화와 staging·journal·final 생성 전에 실행한다. 위반 시 Project ID, entity, Asset ID, 기대·실제 kind와 subject, metadata·참조 수, 필드 목록과 update 등록 방법을 담은 `UNSUPPORTED_INITIAL_PROJECT_ASSETS`를 반환한다. 허용된 create는 staging의 current와 version 0을 쓴 뒤 create transaction ID의 lock을 만들고, Current·Version 0·빈 관리 디렉터리·Asset-free 상태와 lock 소유권을 검사한 상태로 디렉터리를 게시한다. final 검증과 create journal cleanup을 마친 뒤 같은 metadata와 inode인 lock을 마지막으로 제거하고 디렉터리를 sync한 후 반환한다. 이 동안 Update는 `PROJECT_BUSY`이며 transform과 version 1, update journal을 만들지 않는다.

Create recovery는 v3 journal의 owner와 final lock의 Project ID·transaction ID·host·PID가 같아야 commit을 인정한다. 살아 있는 owner는 `PROJECT_BUSY`로 보존하고, 종료된 owner의 완전한 final은 journal을 먼저 정리하고 동일 inode lock을 제거해 `create-committed`로 복구한다. 게시 단계인데 lock이 없거나 lock·journal이 다르거나 final이 불완전하면 자동 삭제하지 않고 `STORE_CREATE_RECOVERY_REQUIRED` marker를 남긴다. 게시 전 중단은 소유 staged lock을 확인해 제거한 뒤 자기 staging과 journal만 rollback한다. 다른 완전한 동시 create는 기존 보수적 `create-superseded` 처리를 유지하며 journal v2도 이전 정책으로 읽는다. 새 create 제한은 기존 Asset-bearing 저장본의 읽기·복구에 적용하지 않는다.
...
6. 두 가지 이상의 구성으로 회귀·브라우저 검증, 전체 요구사항 감사: 27개 파일의 569개 자동 테스트로 합성 자료와 초기 회귀 자료의 가져오기·편집·출력을 검증했다. 정확한 이름의 저장 경계 계약 90개는 Asset reference 여섯 필드, Initial Create의 disk side effect 없는 거부, Next catalog closure, Validator 공통 정책, `EEXIST`·sync·identity 실패, Create lock 게시·정리·복구, 두 Store의 Create–Update 경쟁, transient initialize 재시도, journal v2/v3와 Worker queue, PRJ-007 Golden을 포함한다. 기존의 동일 hash·다른 inode, 모든 version·다른 transaction 참조, symlink, 영속 recovery block과 큰 PCM24 업로드 중 status 응답도 유지한다. PRJ-007 Golden은 실제 48,000Hz 2초 WAV를 `UNIT-045`의 849,000–851,000ms J-cut에 연결한다. 전체 분량의 시각·낭독 검토는 남아 있다.
