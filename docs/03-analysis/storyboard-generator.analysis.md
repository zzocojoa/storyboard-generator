...
SFX·Music과 외부 녹음 Dialogue·Voiceover·Panel은 같은 multipart 등록 경로를 사용한다. mono/stereo 16/24-bit PCM WAV를 파싱해 실제 길이를 적용하고, 프로젝트 샘플레이트의 PCM16 WAV로 정규화한 결과를 다시 검사한다. AIFF·MP3는 지원하지 않는다. 서버는 저장과 읽기 모두에서 파일 존재·경로·SHA-256·MIME·구조를 확인하고 Program Monitor에는 `no-store` 안전 Audio URL만 제공한다.
