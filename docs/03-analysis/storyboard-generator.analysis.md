...
ProjectStore는 Project lock을 current read 전에 원자 획득한다. 소유 lock 아래에서 current와 current version 일치, `expectedRevision`, transform, Asset catalog transition, 다음 version·Asset 경로와 write·hash·미디어를 검사한다. journal 직전에는 lock metadata, current revision·SHA-256, 게시 경로 부재를 다시 확인한다. 두 Store instance가 경쟁하면 하나만 commit하고 다른 쪽은 `PROJECT_BUSY` 또는 `REVISION_CONFLICT`로 끝난다. 두 오류와 일반 transform·Asset 계약 오류는 lock을 제거하며 journal이나 recovery block을 만들지 않는다. transform에는 current의 deep clone을 주고 previous content와 hash는 clone 전 current에서 계산한다.

Asset catalog는 append-only이며 같은 ID의 Asset Schema 전체 metadata가 immutable이다. 기존 Asset 제거, 기존 ID의 필드 변경, 기존 경로 write를 거부한다. 교체는 신규 ID·경로·version·실제 write를 추가하고 기존 항목과 파일을 보존한다. 신규 metadata와 write의 경로 집합은 정확히 1:1이어야 한다. journal version 3은 이전·다음 Project, revision과 신규 Asset의 staged·final 상대경로, SHA-256과 phase를 기록한다. Asset → revision → current 순서로 게시하고 current 교체를 commit point로 사용하며 staged hard link는 cleanup까지 남긴다. Rollback은 staged와 final의 hash와 `dev`·`ino`가 모두 일치해야 게시 소유권을 인정한다. 그 뒤 current, version 0을 포함한 모든 version, 다른 transaction previous·next의 Asset 참조를 검사한다. 같은 hash의 다른 inode, 손상·불일치 version, symlink, 다른 참조가 있으면 삭제하지 않는다. version 2 journal은 게시 전 중단과 완전한 commit만 보수적으로 자동 복구한다.

Initial Create는 Asset metadata와 Frame·Audio Cue·Generation Record·Shot continuity의 모든 Asset 참조가 없는 Project만 허용한다. 위반은 disk side effect 전에 `UNSUPPORTED_INITIAL_PROJECT_ASSETS`로 끝난다. 허용된 create의 복구는 final의 current, version 0, current revision snapshot, 모든 version의 ID·revision·내용과 관리 디렉터리까지 검증한 후 journal을 정리한다. 다른 완전한 동시 create가 먼저 게시됐으면 final을 보존하고 자기 staging만 `create-superseded`로 제거한다. 불완전하면 `STORE_CREATE_RECOVERY_REQUIRED`로 final과 create journal을 보존한다. 과거 Asset-bearing Project는 계속 읽고 복구한다. 모든 관리 파일 연산은 canonical data root, `lstat`·`realpath`, regular-file 검사, `O_NOFOLLOW`를 적용한 `SafeStoreFilesystem`을 통과한다. Project 하위 symlink와 root 탈출을 거부하고 unlink 직전 identity를 재확인한다.
...
현재 로컬 자동 검사는 26개 파일의 491개 테스트를 통과한다. 기존 413개를 그대로 유지하고 lock-before-read와 current 재확인 7개, 동시 update 13개, lock cleanup 7개, initial create 12개, Asset 불변성 19개, transform 격리 6개, HTTP 경쟁 6개, PRJ-007 보강 8개를 추가했다. 기존 JSON round-trip 검사와 합쳐 지정된 79개 이름을 모두 실행한다. 동시성 검사는 Promise barrier와 두 `ProjectStore` instance를 사용하며 sleep에 의존하지 않는다. 실제 writer fault injection, 동일 hash·다른 inode, 모든 version과 다른 transaction 참조, initial create 완전성, symlink, recovery block, queue와 HTTP 동시 응답도 유지한다. PRJ-007 Golden은 12 Scene, 32 Segment, screenplay Unit 79개, Panel Turn 16개, 1,500,000ms와 원문·시간·참조 보존을 검사한다. `SEG-024` Text는 1,088,000ms, 1,108,000ms, 1,148,000ms 공개를 검사하고 Audio는 실제 발화 Source가 지원하는 단계만 보고한다. `UNIT-045`는 저장소 fixture의 실제 48,000Hz mono PCM16 2,000ms WAV를 849,000–851,000ms J-cut으로 저장해 안전 HTTP bytes, JSON 재열기와 Gate 불변성을 검증한다.
