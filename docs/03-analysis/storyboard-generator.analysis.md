...
ProjectStore는 다음 revision과 신규 Asset 경로 충돌, metadata/write/hash 1:1, 안전한 parent를 journal 전에 검사한다. journal version 3은 이전·다음 Project, revision과 Asset의 staged·final 상대경로, SHA-256과 phase를 기록한다. Asset → revision → current 순서로 게시하고 current 교체를 commit point로 사용하며 staged hard link는 cleanup까지 남긴다. Rollback은 staged와 final의 hash와 `dev`·`ino`가 모두 일치해야 게시 소유권을 인정한다. 그 뒤 current, version 0을 포함한 모든 version, 다른 transaction previous·next의 Asset 참조를 검사한다. 같은 hash의 다른 inode, 손상·불일치 version, symlink, 다른 참조가 있으면 삭제하지 않는다. version 2 journal은 게시 전 중단과 완전한 commit만 보수적으로 자동 복구한다.

Initial Create 복구는 final의 current, version 0, current revision snapshot, 모든 version의 ID·revision·내용, 참조 Asset과 관리 디렉터리까지 검증한 후 journal을 정리한다. 다른 완전한 동시 create가 먼저 게시됐으면 final을 보존하고 자기 staging만 `create-superseded`로 제거한다. 불완전하면 `STORE_CREATE_RECOVERY_REQUIRED`로 final과 create journal을 보존한다. 모든 관리 파일 연산은 canonical data root, `lstat`·`realpath`, regular-file 검사, `O_NOFOLLOW`를 적용한 `SafeStoreFilesystem`을 통과한다. Project 하위 symlink와 root 탈출을 거부하고 unlink 직전 identity를 재확인한다.
...
현재 로컬 자동 검사는 25개 파일의 413개 테스트를 통과한다. 기준 321개를 그대로 유지하며 storage 안전성 74개와 Worker queue 11개를 포함해 92개를 추가했다. 지정된 95개 회귀 이름을 모두 포함하며 이전 테스트를 이름만 바꿔 신규 수로 세지 않는다. 실제 writer fault injection은 update·create 12개 지점 모두에서 `ProjectStore`를 실행하고 새 instance의 복구 결과를 확인한다. 수동 disk fixture는 v2 호환과 손상 상태 구성에만 병행한다. 동일 hash version 충돌, 같은 hash·다른 inode, 모든 version과 다른 transaction 참조, initial create 완전성, symlink, recovery block, queue와 HTTP 동시 응답을 검증한다. PRJ-007 Golden은 12 Scene, 32 Segment, screenplay Unit 79개, Panel Turn 16개, 1,500,000ms와 원문·시간·참조 보존을 검사한다. `SEG-024` Text는 1,088,000ms, 1,108,000ms, 1,148,000ms 공개를 검사하고 Audio는 실제 발화 Source가 지원하는 단계만 보고한다. `UNIT-045`는 저장소 fixture의 실제 48,000Hz mono PCM16 2,000ms WAV를 849,000–851,000ms J-cut으로 저장해 849,500ms 선택, 안전 HTTP bytes, JSON 재열기와 Gate 불변성을 검증한다. 메모리에 metadata만 넣는 기존 검사는 도메인 회귀이며 실제 Asset E2E 증거로 계산하지 않는다.
