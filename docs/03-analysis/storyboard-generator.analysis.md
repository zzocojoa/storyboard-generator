...
SFX·Music과 외부 녹음 Dialogue·Voiceover·Panel은 같은 multipart 등록 경로를 사용한다. mono/stereo 16/24-bit PCM WAV를 파싱해 실제 길이를 적용하고, 입력 sample rate·chunk 수와 출력 Frame·Byte·Sample 연산량을 먼저 제한한다. Sample 변환은 수와 실행 시간·메모리가 제한된 Worker Thread에서 수행하고, 완료된 프로젝트 샘플레이트의 PCM16 WAV를 다시 검사한다. AIFF·MP3는 지원하지 않는다. 서버는 저장과 읽기 모두에서 파일 존재·경로·SHA-256·MIME·구조와 실제 WAV의 duration·sample rate·channel·codec을 Asset metadata에 대조하고 Program Monitor에는 Cue 길이까지 맞는 `no-store` 안전 Audio URL만 제공한다. 유효한 이전 24kHz 또는 PCM24 WAV는 손상과 구분해 새 Asset 버전으로 복구할 수 있다.

ProjectStore는 변경에 필요한 이전/다음 Project, revision과 Asset을 journal version 2 transaction으로 staging하고 Asset → revision → 현재본 순서로 게시한다. journal은 각 파일의 SHA-256과 Asset ID·경로, Host·PID·transaction 소유권을 기록한다. 시작 시 현재 revision, journal, 참조 상태가 함께 증명된 경우에만 게시 전 파일을 제거하거나 완전한 commit을 유지한다. 증명이 불완전하면 원본 파일과 lock을 보존하고 `STORE_RECOVERY_REQUIRED`로 중단한다. Initial Project는 create journal 안에서 완전한 디렉터리를 만든 뒤 원자적으로 게시하므로 최종 경로에 부분 Project를 만들지 않는다. 복구용 Asset 경로가 프로젝트의 `assets` 밖으로 나가면 복구를 중단한다. Browser Audio controller는 Cue 종료와 playhead 범위를 모두 적용하며 프로젝트·revision 전환과 이전 비동기 완료가 새 재생을 오염시키지 않게 한다.
