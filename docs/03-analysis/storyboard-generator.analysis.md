...
Generation Record는 별도 전이 정책으로 append-only를 강제한다. 기존 Record를 삭제하면 `GENERATION_RECORD_REMOVAL_FORBIDDEN`, metadata를 바꾸면 `GENERATION_RECORD_IMMUTABLE`, 순서를 바꾸거나 중간에 삽입하면 `GENERATION_RECORD_ORDER_IMMUTABLE`이다. 신규 Record는 배열 끝에만 추가할 수 있으며 Next Project에 없는 Shot은 `GENERATION_RECORD_SHOT_NOT_FOUND`, Next catalog에 없는 결과 Asset은 중앙 closure의 `ASSET_REFERENCE_NOT_FOUND`로 journal 전에 거부한다. 같은 revision에서 신규 Asset과 Record를 함께 추가하는 경로는 허용한다.
...
Startup Create recovery는 `.create-locks`의 root lock부터 검사한다. 같은 Host의 살아 있는 owner는 Project별 Active Create로 보존하며 그 Project의 Create·Update만 Busy이고 다른 Project의 초기화·읽기·변경은 계속된다. 종료된 owner에서 journal이 없고 final도 없으면 pre-journal 중단으로 root lock을 정리한다. matching v3 journal이 있으면 create transaction을 복구하고, 완전한 final이 있으면 journal 유무와 관계없이 Current·Version 0과 lock 관계를 확인해 commit을 보존한다. root lock·journal·final lock이 불일치하거나 다른 Host·손상·불완전 구조이면 `STORE_CREATE_RECOVERY_REQUIRED` marker를 남기고 root lock과 관련 자료를 자동 삭제하지 않는다. 과거 Asset-bearing Project와 journal v2는 기존 읽기·복구 정책을 유지한다. 모든 관리 파일 연산은 canonical data root, `lstat`·`realpath`, regular-file 검사, `O_NOFOLLOW`를 적용한 `SafeStoreFilesystem`을 통과한다. Project 하위와 `.create-locks`의 symlink, root 탈출을 거부하고 unlink 직전 identity를 재확인한다.
...
HTTP 오류는 기존 `code`, `message`, `issues`에 `category`, `retryable`, `operatorActionRequired`를 더한다. Validation 400과 Not Found 404는 자동 재시도하지 않는다. `PROJECT_BUSY`, `REVISION_CONFLICT`, `PROJECT_ALREADY_EXISTS`는 Conflict 409이며 조건이 해소된 뒤 사용자가 다시 시도할 수 있다. `STORE_RECOVERY_*`, `STORE_LOCK_CLEANUP_REQUIRED`, journal·경로 안전 오류는 Storage Locked 423이고 운영자 조치가 필요하다. `STORE_LOCK_ACQUISITION_FAILED`는 Unavailable 503과 retryable=true, 분류되지 않은 오류는 500이다. Web은 Import 중 재클릭을 차단하고, 423 상태에서는 영속 복구 배너와 mutation 잠금을 유지한다.
...
현재 로컬 자동 검사는 28개 파일의 666개 테스트를 통과한다. 지정된 정확한 계약 이름 109개는 Asset reference와 Initial Create closure, Project별 Root Create Lock, 두 Store의 동시 Create, crash recovery, Project 단위 Active Create, Generation Record append-only, HTTP 400·404·409·423·503·500과 Web UI 상태, 기존 저장과 PRJ-007 회귀를 포함한다. 기존에 정확한 이름으로 존재한 12개를 유지하고 97개를 추가했다. 동시성 검사는 Promise barrier와 두 `ProjectStore` instance를 사용하며 sleep에 의존하지 않는다. 실제 writer fault injection, 동일 hash·다른 inode, 모든 version과 다른 transaction 참조, initial create 완전성, symlink, recovery block, queue와 HTTP 동시 응답도 유지한다. PRJ-007 Golden은 12 Scene, 32 Segment, screenplay Unit 79개, Panel Turn 16개, 1,500,000ms와 원문·시간·참조 보존을 검사한다. `SEG-024` Text는 1,088,000ms, 1,108,000ms, 1,148,000ms 공개를 검사하고 Audio는 실제 발화 Source가 지원하는 단계만 보고한다. `UNIT-045`는 저장소 fixture의 실제 48,000Hz mono PCM16 2,000ms WAV를 849,000–851,000ms J-cut으로 저장해 안전 HTTP bytes, JSON 재열기, Gate와 Generation Record 불변성을 검증한다.
