...
위 권한 규칙은 구조화 원본 어댑터에 적용한다. 제작 문서 8개만 사용하는 입력은 별도 `production-documents-v1` 계약으로 지원한다. 문서 행 출처와 새 Unit ID를 보존하고 미해결 인물·장면·구간 연결을 보여 준다. 상세 범위와 검증 조건은 [문서 입력 계획](document-handoff.plan.md)을 따른다. 구조화 입력 실패 시 자동 전환하지 않는다.
