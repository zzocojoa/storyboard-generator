...
function resourceVersions(project: Project, resourceId: string): { assets: Asset[]; error: string } {
  try {
    const ids = new Set(project.generationRecords.filter((record): boolean => ['automatic-production-reference-1.0.0', 'automatic-production-reference-1.1.0'].includes(record.templateVersion))
...
export function ProductionReferenceReview(props: Props): ReactElement | null {
  ...
  return <section className="production-reference-review" aria-label="자동 제작 기준 이미지 검토">
    ...
    {resource !== undefined && <>
      ...
      <button disabled={disabled || overview?.configured !== true || overview.recommendedSettings === null || active !== undefined || currentAsset === undefined || protectedShots.length > 0 || error !== '' || history.error !== ''}
        onClick={(): void => { if (overview?.recommendedSettings !== null && overview?.recommendedSettings !== undefined) void perform(() => startReferenceRetake(props.project.projectId, props.project.revision, { resourceId }, { ...overview.recommendedSettings!, maxJobs: 1, maxAttemptsPerJob: 2, maxImageAttempts: 2, maxActiveMs: 600000, maxStagedBytes: 67108864, audioProduction: 'instructions-only' })); }}>선택 기준만 다시 생성</button>
