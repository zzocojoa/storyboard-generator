...
import { z } from 'zod';
import { AudioInstructionInputSchema, AudioInstructionOccurrenceInputSchema, audioInstructions } from '../../src/domain/audio-instructions.js';
...
import { audioInstructionContentIssues } from '../../src/domain/audio-instruction-evidence.js';
import type { Instruction, Project } from '../../src/domain/schema.js';
import { AudioInstructionOccurrenceSourceSchema } from '../../src/domain/schema.js';
import { AudioOccurrenceEditor } from './AudioOccurrenceEditor.js';
...
const DraftOccurrenceSchema = AudioInstructionOccurrenceInputSchema.extend({ reason: z.string(), source: z.discriminatedUnion('kind', [
  AudioInstructionOccurrenceSourceSchema.options[0].extend({ quote: z.string() }), AudioInstructionOccurrenceSourceSchema.options[1].extend({ quote: z.string() }),
]) });
const DraftSchema = AudioInstructionInputSchema.extend({ resolution: z.enum(['none', 'required']).nullable(), reason: z.string(), occurrences: z.array(DraftOccurrenceSchema).max(128).optional() });
...
function InstructionEditor(props: ReviewProps & { instruction: Instruction }): ReactElement {
  ...
  const recovery = useBrowserDraft(JSON.stringify([props.project.projectId, 'audio-instruction', props.instruction.id]), {
    ...
    ...(saved?.occurrences === undefined ? {} : { occurrences: saved.occurrences }),
  ...
  const reviewIssues = saved === undefined ? [] : audioInstructionContentIssues(props.project, props.instruction, saved);
  return <article aria-label={`${props.instruction.id} 음향 지시 검토`}>
    ...
    {recovery.notice}
    {reviewIssues.map((entry): ReactElement => <p role="status" key={entry.code}>{entry.message}</p>)}
    <fieldset disabled={props.working || recovery.blocked}>
      <label className="field">음향 필요 여부<select value={draft.resolution ?? ''} onChange={(event): void => { const resolution = event.target.value === '' ? null : event.target.value as 'none' | 'required'; recovery.setValue({ ...draft, resolution, sharedScope: null, cueIds: resolution === 'none' ? [] : draft.cueIds, informationIds: resolution === 'none' ? [] : draft.informationIds, ...(draft.sourceEvidence === undefined ? {} : { sourceEvidence: resolution === 'none' ? [] : draft.sourceEvidence }), ...(draft.occurrences === undefined ? {} : { occurrences: resolution === 'none' ? [] : draft.occurrences }) }); }}>
        <option value="">판정 대기</option><option value="required">음향 필요</option><option value="none">현재 구간에 추가 트랙 없음</option>
      </select></label>
      {draft.resolution === 'required' && draft.occurrences !== undefined && <AudioOccurrenceEditor project={props.project} instruction={props.instruction} value={draft.occurrences} onChange={(occurrences): void => {
        const unitIds: string[] = [...new Set(occurrences.flatMap((entry): string[] => [...(entry.source.kind === 'unit' ? [entry.source.unitId] : []), ...(entry.supportingUnitIds ?? [])]))];
        recovery.setValue({ ...draft, occurrences, sharedScope: null, cueIds: occurrences.flatMap((entry): string[] => entry.cueId === null ? [] : [entry.cueId]),
          informationIds: [...new Set(occurrences.flatMap((entry): string[] => entry.informationIds))],
          sourceEvidence: unitIds.flatMap((unitId) => {
            const unit = props.project.dataset.units.find((entry): boolean => entry.id === unitId);
            // 사라진 원문의 작성 중 인용은 보존하며 서버 검증을 우회해 새 원문으로 바꾸지 않는다.
            return unit === undefined ? (draft.sourceEvidence ?? []).filter((entry): boolean => entry.unitId === unitId) : [{ unitId, quote: unit.text }];
          }) });
      }} />}
      {draft.resolution === 'required' && draft.occurrences === undefined && <><p>같은 소리의 기존 트랙을 연결하세요. 선택하지 않고 저장하면 별도 준비 트랙을 만듭니다.</p>
        ...
        <button onClick={(): void => { recovery.setValue({ ...draft, occurrences: [], cueIds: [], informationIds: [], sourceEvidence: [], sharedScope: null }); }}>발생별로 직접 배치</button>
      </>}
      ...
      <button disabled={saved === undefined || saved.reviewStatus === 'confirmed' || recovery.dirty || reviewIssues.length > 0} onClick={(): void => { void props.onConfirm(props.instruction.id); }}>음향 판정 확인</button>
    </fieldset>
