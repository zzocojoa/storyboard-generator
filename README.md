...
**제작 설정 → 기준 이미지 검토·재생성**에서 검토할 제작 기준을 선택합니다. 현재 이미지·설명·연결 구간을 확인하고 **선택 기준만 다시 생성**을 누르면 Codex App이 그 한 장을 같은 설명으로 다시 생성합니다. 기존 자동 실행이 남아 있으면 제작 현황에서 완료하거나 취소한 뒤 시작하세요. 확정·잠금 컷이 사용하는 기준은 먼저 검토 대기로 변경하고 잠금을 해제해야 합니다.
