...
시작 복구는 `.create-locks`도 검사한다. 같은 Host의 살아 있는 Initial Create는 해당 Project에만 활성 상태로 기록하므로 다른 Project의 목록·읽기·수정·생성을 막지 않는다. 종료된 owner의 root lock은 matching create journal과 final lock을 검증해 commit 또는 rollback을 끝낸 뒤 제거한다. journal이 없는 pre-journal 중단과 완전한 final은 안전하게 정리하고, 다른 Host·손상된 lock·transaction 불일치는 lock을 보존한 채 해당 Project에 recovery marker를 남긴다.

Asset catalog와 Generation Record는 append-only다. 같은 Asset ID의 metadata 전체는 revision 사이에서 바꿀 수 없고 기존 Asset을 제거하거나 기존 경로에 새 write를 제출할 수 없다. 기존 Generation Record도 삭제·재정렬하거나 provider·model·prompt·result asset·shot·생성 시각을 포함한 어떤 metadata도 바꿀 수 없으며 새 Record는 배열 끝에만 추가한다. 각 Record의 `shotIds`와 `resultAssetIds`는 같은 revision에서 추가한 대상까지 포함해 실제 Shot과 Asset을 가리켜야 한다. 자산 교체와 Source Update는 과거 Generation Record를 그대로 보존한다.
...
API 오류는 `code`, `message`, `issues`와 함께 `category`, `retryable`, `operatorActionRequired`를 반환한다. 입력 계약 위반은 400, 없는 대상은 404, Busy·revision 충돌·이미 존재하는 Project는 409, 저장 복구가 필요한 Project는 423, 일시적인 lock 획득 실패는 503, 분류되지 않은 서버 오류는 500이다. 웹 화면은 423에서 **STORAGE RECOVERY REQUIRED**를 표시하고 변경 버튼을 잠그며 자동 재시도하지 않는다. 503은 **STORAGE TEMPORARILY UNAVAILABLE**로 표시한다.
