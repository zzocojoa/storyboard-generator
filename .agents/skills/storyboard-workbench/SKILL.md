...
## Result application and reconciliation

Proposal, image and speech registration all use the shared apply coordinator. Register through the existing apply commands; do not call the low-level Request complete primitive with a guessed revision. The coordinator claims the Request logical key before acquiring the Project lock, validates kind/project/target/basis/generation fingerprint, persists an Applying intent, commits the Project, verifies the actual Version introduction and settles the Request. Keep model generation and macOS synthesis outside the Project lock.

An Applying request must never be passed back to generation Pending or marked failed/superseded while its Project commit is unresolved. Use `npm run codex-workbench -- apply-status --request <UUID>` to inspect it and `npm run codex-workbench -- reconcile --request <UUID>` to settle that request. App startup and ordinary CLI commands reconcile interrupted Applying requests. The status command does not execute Apply settlement, but still initializes the ordinary stores; use the read-only Review Bundle path when no source mutation is allowed. HTTP equivalents are GET `/api/codex/requests/:requestId/apply-status` and POST `/api/codex/requests/:requestId/reconcile`.

`resultRevision` is the verified first Version containing that request's Generation Record, never the current Project revision. Reconciliation preserves later edits and uses committed evidence even if the original result file is missing. It does not regenerate media or append duplicate Project revisions, records or assets. A different result for the same request is CODEX_APPLY_RESULT_CONFLICT 409. Legacy Proposal/Speech without an input identity hash requires input-free reconcile instead of guessing whether a supplied file matches. The completed intent remains as input identity audit evidence; do not delete it as cleanup.

Distinguish applying, committed-awaiting-request-settlement, completed, recovery-required and evidence-conflict. Report Request ID, Project ID and the verified committedRevision. CODEX_REQUEST_APPLY_IN_PROGRESS is retryable 409, not a persistent Project recovery block. CODEX_APPLY_RECOVERY_REQUIRED is request-scoped retryable 423; evidence conflicts and unresolved receipts are request-scoped 423 requiring review. These do not lock unrelated Project mutations. Preserve unknown/foreign intents and legacy Failed/Superseded-plus-record or incorrect Completed revision conflicts; never silently rewrite them. Review Bundle creation must not invoke Apply recovery. Applying is excluded from ordinary Pending and generation failure metrics.

Project Schema 1.9.0, Build Provenance 3 and Project Journal/Lock/Registry 3/3/1 remain unchanged. Legacy unversioned Request JSON and Journal 1 stay readable without parse-time disk rewriting or relabeling historical Build metadata. The commit proof comes from the existing atomic Project Version history, not a later separate receipt write. Guarantee at most one stored result application for cooperating writers on local macOS/Ubuntu filesystems; do not claim exactly-once external AI calls, arbitrary file editing or distributed SMB/NFS writers. Completion still requires separate Text Confirm, Frame Accept, Shot Approval and Final Readiness checks.
