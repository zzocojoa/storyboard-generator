...
Normal bridge commands prepare a Build Manifest before execution. New requests and Generation Records retain the actual generatorBuild, and processing requires the current source Build to match the queued Build. A missing or stale Build requires a fresh request. Never assign the current Commit to an older generation: Schema 1.7 migration preserves old records with generatorBuild=null and leaves original Version files unchanged. `/api/status.build` identifies the running code, while the asset manifest connects each generated asset to its generation Build and audit record.
...
Use `--data-root` for an explicitly selected existing store. Output must be a new directory outside that Data Root. Final failure returns all blocking issues without creating a success bundle. The nine base files include project, PDF, CSV, readiness, generation audit, asset integrity, asset manifest, build manifest, and checksum manifest. Separate source media bytes are included only with explicit `--include-media`; the PDF contains its storyboard images. The reader verifies Current, all Versions, and assets before and after the export without initialization, recovery, locks, or heartbeat writes. Preserve unknown historical Build values and report existing timing differences instead of rewriting them.

Current audit deduplicates revisions, rejects a stable Current-Version mismatch, records only absent-to-present reappearance, and checks targets from their introduction revision. `/api/status.activeUpdateErrors` retains unverifiable active updates with project-scoped recovery. A successful recovery removes the stale error; do not hide it manually or block unrelated projects.
